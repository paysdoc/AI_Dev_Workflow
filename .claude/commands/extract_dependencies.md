...
See also: `/extract_touched_paths` — a companion command that extracts likely-touched
file paths from an issue body (used by the region-overlap serialization gate to detect
when two issues would edit the same file and should be serialized rather than spawned
in parallel).
