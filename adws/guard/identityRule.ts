 ...
 *
 * Two constructor shapes are inspected: `gitContextForRepo(x)` — the
 ...
 * whether a declaration still exists for them.
 *
 * `readLocalRepoIdentity` (`adws/core/localRepoIdentity.ts`) joined
 ...
 *
 * Known, unchanged limitation: matching is by identifier text, so an
 ...
 * them is cwd-derived identity. `getRepoInfo` and `readLocalRepoInfo` stay in
 * this set with no declaration left: this is a name-based AST match against
 * identifier text, not a file reference, so retaining a retired name is what
 * stops a cwd-derived identity fallback of that name being reintroduced
 * later. Only `SANCTIONED_CONSTRUCTION_SITES` (constructionRule.ts) carries a
 * stale-entry ratchet — nothing here fails because a guarded name has no
 * declaration left.
 */
export const CWD_DERIVED_IDENTITY_FNS = new Set(['getRepoInfo', 'readLocalRepoInfo', 'readLocalRepoIdentity']);

/** The context constructors whose identity argument this rule inspects — name-based, surviving both retired declarations. */
 ...
 * (`healthCheck.tsx`'s `readLocalRepoIdentity(REPO_ROOT)`) and passes.
