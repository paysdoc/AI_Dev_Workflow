/**
 * Two initializeWorkflow calls with the same adwId/issueNumber/issueType and a
 * branch-name agent that would return divergent slugs MUST result in exactly
 * one branch being created and the agent being invoked exactly once.
 */
...
// assert the boundary-providers passthrough deterministically.
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        expect(cfg2.branchName).toBe(branchA);
      } finally {
        cleanupAdwId(adwId2);
      }
    },
    ...
    async () => {
      ...
      try {
        ...
        mockGitCtx.getWorktreeForBranch
          ...
          .mockReturnValueOnce(FAKE_WORKTREE_PATH);
...
describe('initializeWorkflow: boundary-providers passthrough to bindWorkspaceContext', () => {
  ...
  it('refuses to mint a second provider set when a caller-supplied repoId names a different repository — bindWorkspaceContext is never reached', async () => {
    ...
    // A contradicting caller-supplied identity is refused (resolveWorkflowProviders throws,
    // caught by initializeWorkflow's surrounding try/catch) rather than served a second,
    // ad-hoc-bound workspace — bindWorkspaceContext is never called and cfg.repoContext
