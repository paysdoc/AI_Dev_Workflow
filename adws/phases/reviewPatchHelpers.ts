...
export async function applyPatchBlocker(
  ...
  const { adwId, logsDir, specFile, worktreePath, issue, orchestratorStatePath, subprocessEnv, launchContext } = ctx;
  ...
  const patchResult = await runPatchAgent(adwId, blocker, logsDir, specFile, undefined, patchStatePath, worktreePath, issue.body, subprocessEnv, launchContext);
  ...
  if (patchResult.success) {
    ...
    const buildResult = await runBuildAgent(issue, logsDir, patchResult.output, undefined, buildStatePath, worktreePath, subprocessEnv, launchContext);
...
export async function applyRefactorBlockers(
  ...
  const { adwId, logsDir, worktreePath, issue, orchestratorStatePath, subprocessEnv, launchContext } = ctx;
  let costUsd = 0;
  ...
  for (const blocker of blockers) {
    ...
    const refactorResult = await runRefactorAgent(adwId, blocker, logsDir, refactorStatePath, worktreePath, issue.body, subprocessEnv, launchContext);
    ...
    const buildStatePath = AgentStateManager.initializeState(adwId, 'build-agent', orchestratorStatePath);
    const buildResult = await runBuildAgent(issue, logsDir, refactorResult.output, undefined, buildStatePath, worktreePath, subprocessEnv, launchContext);
