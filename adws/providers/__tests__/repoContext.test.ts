...
describe('loadProviderConfig', () => {
  ...
  it('parses jira as issue tracker platform', () => {
    vi.mocked(existsSync).mockReturnValue(true);
    vi.mocked(readFileSync).mockReturnValue(
  ...
  it('parses jira case-insensitively', () => {
    vi.mocked(existsSync).mockReturnValue(true);
    vi.mocked(readFileSync).mockReturnValue(
      '## Code Host\ngithub\n\n## Issue Tracker\nJira\n',
    );
...
describe('resolveIssueTracker', () => {
  ...
  it('returns Jira issue tracker when platform is jira', () => {
    const mockJiraTracker = { _type: 'jira-issue-tracker' };
    vi.mocked(createJiraIssueTracker).mockReturnValue(mockJiraTracker as unknown as ReturnType<typeof createJiraIssueTracker>);
    ...
    const tracker = resolveIssueTracker('jira', validRepoId, config);

    expect(createJiraIssueTracker).toHaveBeenCalledWith('https://jira.example.com', 'PROJ');
    expect(tracker).toBe(mockJiraTracker);
  });
  ...
  it('throws when jira is selected but issueTrackerUrl is missing', () => {
    ...
    expect(() => resolveIssueTracker('jira', validRepoId, config)).toThrow(
