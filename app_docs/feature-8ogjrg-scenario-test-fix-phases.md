# Scenario Test Phase + Scenario Fix Phase (BDD Coverage)

**ADW ID:** 8ogjrg-scenariotestphase-sc
**Date:** 2026-04-08
**Specification:** specs/issue-399-adw-1bg58c-scenariotestphase-sc-sdlc_planner-scenario-test-fix-phases.md

## Overview

Introduces two new workflow phases — `scenarioTestPhase` and `scenarioFixPhase` — wired into `adwSdlc.tsx` between step-definition generation and the review phase. BDD scenario execution is extracted from the review retry loop into a dedicated phase with its own orchestrator-level fix-and-retry loop. This run contributed the BDD feature file (`scenario_test_fix_phases.feature`), its Cucumber step definitions, and scenario alignment to the `ScenarioProofResult` shape.

## What Was Built

- `features/scenario_test_fix_phases.feature` — BDD acceptance scenarios covering all acceptance criteria for the new phases (dev-server branching, graceful skip, retry loop, review decoupling, renames)
- `features/step_definitions/scenarioTestFixPhasesSteps.ts` — Cucumber step definitions for the scenario test/fix phase scenarios
- `adws/phases/scenarioTestPhase.ts` — new phase: reads tag-filtered scenario command, optionally wraps in `withDevServer`, runs `runScenarioProof`, returns structured `ScenarioProofResult`
- `adws/phases/scenarioFixPhase.ts` — new phase: takes failures from `scenarioTestPhase`, invokes `runResolveScenarioAgent` per failed tag, commits and pushes
- `adws/phases/__tests__/scenarioTestPhase.test.ts` — unit tests covering dev-server decision, tag filter, graceful skip, pass/fail result shapes
...
- Orchestrator-level retry loop in `adwSdlc.tsx` bounded by `MAX_TEST_RETRY_ATTEMPTS`
...
- `adws/adwSdlc.tsx`: replaced `executeTestPhase` with `executeUnitTestPhase`; added `executeStepDefPhase`, `executeScenarioTestPhase`, `executeScenarioFixPhase`; implemented scenario test/fix retry loop; patched `scenariosMd` to empty before calling review; added `scenarioRetries` to state metadata
- `adws/phases/index.ts`: added exports for `executeScenarioTestPhase`, `executeScenarioFixPhase`; replaced `executeTestPhase` with `executeUnitTestPhase`
...
- All orchestrators (`adwChore`, `adwTest`, `adwPlanBuild*`): updated `executeTestPhase` → `executeUnitTestPhase` import (no new phases added)
...
- `features/scenario_test_fix_phases.feature` — BDD acceptance scenarios
- `features/step_definitions/scenarioTestFixPhasesSteps.ts` — Cucumber step definitions

### Key Changes

- **Decoupled scenario execution from review**: `adwSdlc` review phase no longer runs scenarios; it reads the pre-written `scenario_proof.md` (passed via empty `scenariosMd`)
- **Orchestrator-level retry loop**: `scenarioTest → scenarioFix → scenarioTest` repeats up to `MAX_TEST_RETRY_ATTEMPTS`; on exhaustion the loop exits without hard-failing
- **Dev-server conditional**: `scenarioTestPhase` checks `projectConfig.commands.startDevServer`; if non-`N/A`, wraps `runScenarioProof` in `withDevServer` using the port extracted from `config.applicationUrl`
...
- `scenarioFixPhase` accumulates cost across all failed tags; `scenarioTestPhase` itself reports zero Claude Agent cost (subprocess-only)
...
- See `app_docs/feature-1bg58c-scenario-test-fix-phases.md` for an earlier documentation pass on the same feature
