...
- `evaluateCandidate` (`takeoverHandler`): the single decision tree for any candidate arriving at an issue. Attempts to acquire the spawn lock; if held by a live process, returns `defer_live_holder`. Otherwise reads state: no adwId or no state file → `spawn_fresh`; completed/discarded → `skip_terminal`; paused/paused_auth → `skip_terminal`; abandoned → worktree reset + remote reconcile → `take_over_adwId`; running/starting/resuming with live PID → SIGKILL + reset + reconcile → `take_over_adwId`; running with dead PID → reset + reconcile → `take_over_adwId`; unknown stage → `spawn_fresh`.
