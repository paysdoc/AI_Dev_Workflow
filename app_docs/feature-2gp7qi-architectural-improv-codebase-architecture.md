...
**ADW ID:** 2gp7qi-architectural-improv
**Date:** 2026-03-22
...
- `adws/core/phaseRunner.ts`: New — declarative phase composition engine with `CostTracker`, `PhaseFn`, `runPhasesSequential()`, `runPhasesParallel()`
- `adws/core/environment.ts`: New — dotenv loading, path resolution, directory constants extracted from `config.ts`
- `adws/core/modelRouting.ts`: New — slash command model/effort maps and routing helpers extracted from `config.ts`
- `adws/core/adwId.ts`: New — `generateAdwId()` and `slugify()` extracted from `utils.ts`
- `adws/core/logger.ts`: New — `log()`, `LogLevel`, `LOG_PREFIXES` extracted from `utils.ts`
- `adws/core/claudeStreamParser.ts`: New — moved from `agents/jsonlParser.ts`, reduced `jsonlParser.ts` to a thin re-export
- `adws/core/config.ts`: Trimmed to retry constants, token budgets, comment flags, provider secrets only
- `adws/core/utils.ts`: Emptied and deleted — all exports moved to focused files
- `adws/core/index.ts`: Updated barrel to re-export from all new focused modules
- `adws/agents/commandAgent.ts`: New — shared `runCommandAgent<T>()` for thin wrapper agents
...
- `adws/types/issueRouting.ts`: New — routing maps extracted from `issueTypes.ts`
- `adws/types/issueTypes.ts`: Routing maps removed; file now contains only type definitions
- `adws/cost/commitQueue.ts`: Moved from `core/costCommitQueue.ts`
- `adws/phases/workflowLifecycle.ts`: Removed redundant re-export barrel
- `adws/adwSdlc.tsx`: Reduced from 178 lines of imperative boilerplate to declarative phase list using `PhaseRunner`
