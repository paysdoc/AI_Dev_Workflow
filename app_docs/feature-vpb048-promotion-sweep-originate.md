...
- This module intentionally does not wire `runPromotionSweep` into `trigger_cron.ts` — running it against a live repo creates real commits/issues and must only be done by hand against a disposable test repo, never as part of `bun run test:unit`.
