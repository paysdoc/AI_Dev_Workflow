...
Both `healthCheckChecks.ts` and `healthCheck.tsx` have been removed from the `ALLOWLIST` and the "diagnostic (permanent)" category is closed. `bun run lint:git-guard` now passes with all diagnostic files scanned. The residual `ALLOWLIST` cohort after #699: `adwPromotionSweep.tsx` (plus permanent bootstrap entries only — no diagnostic entries remain).
