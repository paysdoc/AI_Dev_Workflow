...
- **`runScenariosByTag` field in `CommandsConfig`** — Config field parsed from `## Run Scenarios by Tag` in `.adw/commands.md`; defaults to `N/A` (skip).
...
- `adws/agents/index.ts` — Barrel exports updated to expose `runScenariosByTag`, `BddScenarioResult`, `runBddScenariosWithRetry`, `BddScenarioRetryOptions`.
- `adws/core/projectConfig.ts` — Added `runScenariosByTag` to `CommandsConfig`, `'run scenarios by tag'` to `HEADING_TO_KEY`, and `parseUnitTestsEnabled` function.
...
- `.adw/commands.md` — `## Run Scenarios by Tag` section used for issue-scoped BDD scenario execution.
...
- **Graceful skip**: When `runScenariosByTag` is `N/A` (or empty), `bddScenarioRunner.ts` returns `allPassed: true` immediately — the PR proceeds without failing.
...
1. **Configure the BDD scenario command** in the target repo's `.adw/commands.md`:
   ...
   ```
   The `{tag}` placeholder is replaced at runtime with `adw-{issueNumber}`.
...
|---|---|---|---|
| `.adw/commands.md` | `## Run Scenarios by Tag` | `cucumber-js --tags "@{tag}"` | Command template to run tag-filtered BDD scenarios |
| `.adw/project.md` | `## Unit Tests` | absent = `disabled` | Opt-in to running unit tests before BDD scenarios |

When `runScenariosByTag` is `N/A` or absent, scenarios are skipped gracefully and the PR is created unconditionally (after any unit test gate, if enabled).
...
- ADW's own `.adw/commands.md` uses `## Run Scenarios by Tag` with `bunx cucumber-js --tags "@{tag}"` — issue-scoped scenarios are run by substituting `adw-{issueNumber}` for `{tag}`.
