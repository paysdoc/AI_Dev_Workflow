...
- `initializeWorkflow` constructs exactly one `LaunchBoundary` per orchestrator process (`buildLaunchBoundary`) — a `GitContext` plus the forge provider pair (`IssueTracker`/`CodeHost`) bound to the same repo identity. `resolveWorkflowProviders(boundary, callerRepoId?)` is the sole authority for which `RepoIdentifier`/`BoundProviders` pair the resulting `RepoContext` uses, and `bindWorkspaceContext(boundary, worktreePath, repoId)` (`adws/core/launchGitContext.ts`) builds that `RepoContext`. Every downstream phase reaches issue/PR/label/board operations through this provider pair, never through a forge-semantic call on a bare `GitContext`. Since #820, the boundary is built FIRST in the pre-flight (right after the Claude CLI check) and is the sole identity source for the run's issue fetch (`fetchIssueRecord(boundary.gitContext, issueNumber)`, `adws/core/issueRecord.ts`) and the GitHub-App startup validation (`isGitHubAppConfigured` from `adws/core/githubAppAuth.ts`) — no local-git-remote read precedes it.
- `resolveWorkflowRepoId(config)` (`adws/phases/workflowRepoIdentity.ts`, #820): replaces every phase's own `?? getRepoInfo()` wrong-repo-fallback idiom with one precedence over the identity a `WorkflowConfig` already carries — `repoContext.repoId` → `gitContext` → `targetRepo`; all three agree by construction (`bindWorkspaceContext` refuses a mismatch), so precedence only decides which object is read, never which repository is addressed. Throws (rather than falling back to the ambient local git remote) when a config carries none of the three — `initializeWorkflow` always sets at least one, so this is a programming-error guard, not a runtime branch. Used by `orchestratorLock.ts`, `depauditSetup.ts`, `buildPhase.ts`/`documentPhase.ts`/`scenarioFixPhase.ts`/`reviewPhase.ts`'s worktree-context calls, and `prReviewPhase.ts`'s commit-push phase.
...
- `runUpgradeGate`: compares the framework hash against the stored `.adw-version`; on mismatch, atomically claims the upgrade (or attaches to an existing one), parks the current issue with a dependency on the upgrade tracking issue, and returns `{ action: 'parked' }`. On match returns `{ action: 'proceed' }`. Runs after the target repo workspace is resolved but before feature-worktree setup, so a loser parks without ever creating a throwaway worktree; `buildDefaultUpgradeGateDeps(providers, worktreePath, gitCtx)` sources every forge dependency from the provider pair and derives its own git-show thunk from the passed `GitContext`. Since #820, `UpgradeGateParams` carries no `repoInfo` — `UpgradeGateDeps.claimUpgrade: (hash) => Promise<UpgradeClaimResult>` wraps `claimUpgradeOrFindExisting(hash, buildDefaultUpgradeClaimDeps(worktreePath, gitCtx, providers.codeHost))`, and `buildDefaultUpgradeClaimDeps`'s `resolveIssueNumberFromPR` reads the port's `PullRequest.linkedIssueNumber` instead of the legacy `PRDetails.issueNumber` field.
