...
- `hasRegressionPromotionLabel(labels)` (`adws/core/adwLabels.ts`; lived in the now-deleted `adws/github/labelManager.ts` before #821) — pure predicate gating the whole step on the driving issue carrying the `regression-promotion` label (`ADW_REGRESSION_PROMOTION_LABEL`).
