...
- Threads `eventGitContext` through all spawn paths: `classifyAndSpawnWorkflow` → `evaluateCandidate` (worktree/takeover path resolution), `routeIssueOpened` (issues.opened router), and `handleIssueClosedDependencyUnblock` (closed-event dependency unblocking, in `issueClosedUnblockRouter.ts`).
...
- **Two-context isolation is proven at the command boundary:** `webhookRepoResolver.test.ts` injects a recording `exec` into each context's `GitContextDeps` and asserts that each context's `defaultBranch()` records its own token — including immunity to a mid-flight `process.env.GH_TOKEN` overwrite (vestmatic #181 bleed) and to interleaved two-repo execution. As of issue #775, `defaultBranch()` is a repo-API `gh` call and always spawns at the injected framework repo root, so it no longer carries per-repo **cwd** signal; the per-event/per-repo **cwd** isolation assertions (§3a "recorded command cwd equals the context basePath", §4 "each context records its own basePath as cwd") were re-pointed at `getCurrentBranch()` — a git op, which is what actually resolves a cwd per repo. Any new cwd-isolation assertion for a `gh` operation must use a git op for the same reason.
