...
- Three verbatim Markdown table blocks (one per class) inlined in the prompt so the agent writes exact content without LLM paraphrasing
...
- `adwInit.tsx` updated to compute `frameworkRepoRoot` from `import.meta.url` and pass it as the 4th positional arg (`$3`) to `/adw_init`
...
- `.claude/commands/adw_init.md`: Added `frameworkRepoRoot: $3` variable declaration; extended step 7 with `**Draft the observability-surfaces examples block**` sub-bullet (classification rules, three block bodies, Edit-tool replacement instruction); extended step 8 report with class and outcome bullet
...
- `test/fixtures/jsonl/manifests/adw-init-drafts-playwright-examples.json`: Manifest stub simulating the agent writing `features/regression/vocabulary.md` with the browser-test-equipped block (DOM + screenshot rows present)
- `test/fixtures/jsonl/manifests/adw-init-drafts-cli-examples.json`: Manifest stub for the CLI-only block (state files, recorded requests, exit codes, log streams; no DOM/screenshot rows)

### Key Changes

- **Prompt-only on the framework side**: no new TypeScript modules; the `adwInit.tsx` change is purely wiring `frameworkRepoRoot` as `$3`, completing the slice-#507 contract
...
- **Placeholder-missing guard**: if the file is absent or pre-edited, the drafting sub-step skips silently and logs a warning in the step 8 report
...
4. Reports the class chosen and replacement outcome in the step 8 summary

No manual steps required. The `frameworkRepoRoot` argument is always passed by `adwInit.tsx`; legacy invocations without `$3` skip the template copy and drafting and log a warning.

## Configuration

No new environment variables or `.adw/` configuration required. `adwInit.tsx` derives `frameworkRepoRoot` from `import.meta.url` at runtime — it always resolves to the ADW framework repo root regardless of where the orchestrator is invoked from.
...
Additional validation commands from `.adw/commands.md`:
...
- The drafted block is **not** LLM-generated at init time — the agent picks one of three checked-in blocks based on the classification result. This bounds output variance and keeps the BDD structural-conformance assertion stable across framework iterations.
- **Blocked by slice #507** (`feature-nnny1e-vocabulary-template-and-flags`): the template-copy bullet must run first so the placeholder exists to be replaced. If slice #507 has not landed, the drafting sub-step skips silently with a step-8 warning.
...
- Backporting to existing target repos that pre-date this slice is explicitly out of scope.
- See also: `app_docs/feature-mqwyb7-llm-draft-observability-examples.md` — earlier ADW run documenting the same feature.
