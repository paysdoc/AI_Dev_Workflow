...
**Since #819, nothing under `adws/providers/github/` imports this layer**, and since #820 no orchestrator, phase, core utility or proof module does either — `issueApi.ts`, `prApi.ts`, `labelManager.ts`, `projectBoardApi.ts`, and `issueListApi.ts` are now framework-only wrappers reached exclusively by the trigger callers (`adws/triggers/`, #821's remit) and the three legacy-backed adapter closures #820 pushed into `takeoverHandler.ts`/`webhookGatekeeper.ts`/`cancelHandler.ts` to keep `adws/core/` and the orchestrators clean without touching those callers' own boundary-less call shapes. The pure ADW label vocabulary (`ADW_*_LABEL` constants, `ADW_LABEL_DEFINITIONS`, `readAdwLabelNames`, `shouldSkipScenarioAuthoring`, `hasRegressionPromotionLabel`, `hasWontFixLabelName`, …) moved to `adws/core/adwLabels.ts` (#820); `labelManager.ts` and `prApi.ts` re-export it verbatim so this layer's own callers are unchanged. The `GITHUB_APP_*` env wrapper moved to `adws/core/githubAppAuth.ts`; `adws/github/githubAppAuth.ts` is now a three-line re-export shim kept only for `gitContextFactory.ts`. `prCommentDetector.ts`'s `getUnaddressedComments` is now a thin wrapper over the forge-neutral composite `adws/core/unaddressedComments.ts`'s `readUnaddressedComments`, kept alive solely for `trigger_cron.ts`. The duplication with the port classes is deliberate and bounded: #821 migrates the trigger callers onto the ports and deletes this layer's free functions (and their tests) once nothing calls them anymore.
...
- `fetchPRReviewComments` / `fetchPRReviews`: fetch line-level and review-body comments for a PR. `fetchPRReviews(prNumber, repoId)` — the `repoId` parameter comes second (#817, the last call site to widen from a bare `{owner, repo}`-shaped `gh(...)` call).
...
- Every `repoInfo` parameter in this layer is typed `RepoIdentifier` (`adws/providers/types.ts`), not the old `RepoInfo` — the name survived a rename this layer never made, the type it pointed at collapsed into the provider package's shape instead (#817). The raw payload shapes this layer consumes (`GitHubIssue`, `IssueCommentSummary`, `PRDetails`, `PRReviewComment`, `PRListItem`, `RawPR`) are imported from the adapter's `adws/providers/github/domain/` modules, not declared here.
