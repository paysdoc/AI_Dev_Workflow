...
- Computes a unit-test verdict (`pass` / `warn` / `hard-fail`) from four boolean/numeric signals: whether tests are enabled, whether a report was emitted, whether failures exist, and how many test cases ran.
...
- `readJUnitReport` returns `null` for a missing file or a parse error; it logs an error only on parse failure, not on absence.
...
- `computeTestVerdict` treats a report present with zero test cases as `hard-fail` (discovery break), not `warn`.
