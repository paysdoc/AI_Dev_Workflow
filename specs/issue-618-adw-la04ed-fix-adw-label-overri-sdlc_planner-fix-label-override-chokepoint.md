...
- **Validation layering (plan ↔ scenarios):** the fix code has no *runtime* dependency on the BDD harness, but the `@adw-618` scenarios in `features/per-issue/feature-618.feature` encode this issue's acceptance criteria as behaviours (single-label override with no LLM call across all four classification types and both previously-bypassing paths, conflict/absence fall-through to the LLM, and the type-check gate) and are part of the acceptance gate. Validation therefore runs both the `@adw-618` BDD suite and the primary gates — the vitest unit tests (which pin the chokepoint in isolation, consistent with how `issueClassifier` is already covered), type-check, lint, and build. Both layers must pass.
