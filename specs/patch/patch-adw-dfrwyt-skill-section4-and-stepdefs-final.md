# Patch: Enhance SKILL.md Section 4 unit test workflow + create @adw-308 step definitions

## Metadata
adwId: `dfrwyt-unit-test-support-in`
reviewChangeRequest: `Issue #1: SKILL.md Section 4 ('Unit Tests - Conditional') has NOT been enhanced. The file .claude/skills/implement-tdd/SKILL.md has zero diff against origin/dev. The spec requires expanding Section 4 to integrate unit tests into the red-green-refactor loop (RED: write step def + unit test, GREEN: verify both pass), add test framework awareness via .adw/commands.md, reference tests.md and mocking.md in the unit test context, and add an independence reminder. None of this was implemented. Resolution: Enhance SKILL.md Section 4 as described in the spec: expand RED phase to write unit tests alongside step definitions when enabled, expand GREEN phase to verify both pass, add .adw/commands.md reference for test runner, and integrate unit test mentions into the Section 3 loop description.`

## Issue Summary
**Original Spec:** `specs/issue-308-adw-dfrwyt-unit-test-support-in-sdlc_planner-tdd-unit-test-support.md`
**Issue:** SKILL.md has zero diff against origin/dev. Section 4 ("Unit Tests - Conditional") is a minimal 3-line stub that checks `.adw/project.md` but doesn't integrate unit tests into the RED/GREEN/REFACTOR loop. Section 3 makes no mention of unit tests. Additionally, the step definitions file for `@adw-308` scenarios does not exist, so all 12 BDD scenarios fail with undefined steps.
**Solution:** (1) Enhance SKILL.md Section 3 with conditional unit test mentions in RED/GREEN/REFACTOR phases and vertical slicing, (2) rewrite Section 4 with comprehensive enabled/disabled paths, (3) create step definitions file with all 24 steps needed by the `@adw-308` scenarios.

## Files to Modify
Use these files to implement the patch:

- `.claude/skills/implement-tdd/SKILL.md` — Enhance Section 3 (integrate unit test conditional mentions in RED/GREEN/REFACTOR and vertical slicing) and rewrite Section 4 with comprehensive enabled/disabled workflow.
...
5. `Then('the check happens before or during the TDD loop, not after', () => ...)`
   ...
   - Assert the Unit Tests index < Report index
...
9. `Then('there is no separate post-loop section for writing all unit tests at once', () => ...)`
   - Assert no heading (line starting with `#`) contains "all unit tests" batch language
...
12. `Then('it describes skipping unit tests when the {string} setting is {string}', (setting, value) => ...)`
    - Assert content includes `skip unit tests` (case-insensitive) AND `disabled` (case-insensitive)
...
16. `Then('it references {string} for guidance on writing good unit tests', (file) => ...)`
    - Assert content includes the file AND `unit test` in proximity

17. `Then('it references {string} for guidance on mocking in unit tests', (file) => ...)`
    - Assert content includes the file AND `mock` in proximity
...
21. `Then('the enabled-unit-test loop follows this structure:', (dataTable) => ...)`
    ...
    - For RED: assert content mentions `unit test` + `step definition` in RED context
    - For GREEN: assert content mentions both passing in GREEN context
    - For REFACTOR: assert content mentions keeping both green

22. `Then('the disabled-unit-test loop follows this structure:', (dataTable) => ...)`
    - Iterate rows, assert each phase present
    - Assert disabled path describes only BDD scenarios
...
1. Run `NODE_OPTIONS="--import tsx" bunx cucumber-js --tags "@adw-308" --dry-run` — Confirm zero undefined steps
