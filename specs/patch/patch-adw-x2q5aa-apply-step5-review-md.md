# Patch: Insert Step 5 Step Definition Independence Check into review.md
...
**Issue:** `.claude/commands/review.md` was not modified. The spec requires a "Step 5: Step Definition Independence Check" section between Step 4 (Coding Guidelines Check) and the Issue Severity Reference section. The file is identical to origin/dev — the primary deliverable of issue #307 is missing.
**Solution:** Insert the Step 5 section into `.claude/commands/review.md` between Step 4 and `## Issue Severity Reference`, using the content defined in the prior patch spec `specs/patch/patch-adw-yxq5og-add-step5-review-md.md`.

## Files to Modify
Use these files to implement the patch:

- `.claude/commands/review.md` — Insert new `## Step 5: Step Definition Independence Check` section between Step 4 and Issue Severity Reference
...
- Locate the end of `## Step 4: Coding Guidelines Check` (ends after the line `- Report violations as \`tech-debt\` reviewIssues with the specific guideline and file/line location`)
- Insert the following markdown block **after** Step 4's last line and **before** `## Issue Severity Reference`:
...
## Patch Scope
**Lines of code to change:** ~35 lines added to review.md
**Risk level:** low
