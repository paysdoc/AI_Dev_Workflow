...
<!-- ADW-WARNING: The GitHub issue (#448) "Suggested fix directions" say `cwd: entry.worktreePath`, and the BDD scenario `resumeWorkflow passes cwd entry.worktreePath to spawn` in features/pause_queue_resume_spawn_failure.feature encodes that literally. This plan deviates to `cwd: process.cwd()` because target-repo worktrees (e.g. /Users/martin/projects/paysdoc/depaudit/.worktrees/*/) do not contain `adws/` scripts — verified by `ls`. Implementing the issue's literal suggestion would cause the orchestrator to fail at module load with `Cannot find module 'adws/adwSdlc.tsx'` for every external-repo resume. Human reviewer must decide: (a) accept this deviation and update the scenario to match `cwd: process.cwd()`, or (b) reject the deviation and adopt `cwd: entry.worktreePath` (knowingly breaking external-repo resume). Left unresolved in this alignment pass. -->
