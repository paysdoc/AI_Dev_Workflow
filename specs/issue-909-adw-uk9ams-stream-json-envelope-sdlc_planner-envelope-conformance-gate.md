...
4. **Checker.** Fix the trailing-newline bug, accept multi-line fixtures (a whole session excerpt per file), resolve the schema entry per line, assert parser behaviour per event type present (`assistant` → `turnCount`; any `result/*` → `lastResult`; `rate_limit_event` with `status: "rejected"` → `rateLimitRejected`; `system/api_retry` → parses without throwing, no flag asserted because the enum alignment belongs to the detection slice), and check the stub's envelope directory as well as `adws/jsonl/fixtures/`.
5. **Fixtures.** Real captures: the rate-limited session excerpt (`session-rate-limited.jsonl`, multi-line: `system/init` → `rate_limit_event` rejected → `assistant` limit text → `result` with `is_error: true`, `api_error_status: 429`, `terminal_reason: "api_error"`), `system-api-retry-overloaded.jsonl` (529, `error: "overloaded"`), `system-api-retry-rate-limit.jsonl` (429, `error: "rate_limit"`), a real `error_during_execution` result replacing the stale `result-error.jsonl` content, and a snake_case `result-success.jsonl`. The two `assistant` fixtures are backfilled with the fields now required.
...
- `adws/jsonl/schemaProbe.ts` — the live probe. Args gain `--verbose --max-turns 1 --model haiku`; spawns in a throwaway temp cwd so the repo's `.claude/settings.json` hooks and CLAUDE.md are not loaded; gains merge-preserving refresh and the read-only `--check` mode; records `cliVersion` from `system/init.claude_code_version`. Keep under 300 lines by moving pure helpers into the two new modules below.
...
- `adws/jsonl/fixtureUpdater.ts` — simplified to add-missing-required-fields per line.
...
- `adws/jsonl/fixtures/result-error.jsonl` — same file name (the acceptance criterion names "the error fixture"), content replaced by a real `error_during_execution` capture (`is_error: true`, `errors: [...]`, `terminal_reason: "aborted_streaming"`, no `result` field).
...
- Source captures on this host (read-only; outside the repo, never committed as-is):
  ...
  - Any capture with `"subtype":"error_during_execution"` (e.g. `grep -rl '"subtype":"error_during_execution"' agents --include=output.jsonl | head -1`) — a real error result with `errors` and `terminal_reason: "aborted_streaming"` or `"aborted_tools"`.
...
- `test/fixtures/jsonl/envelopes/rate-limit-event-rejected.jsonl`, `test/fixtures/jsonl/envelopes/result-rate-limited.jsonl` — stub envelope templates for the rate-limited response.
...
- Unit tests in `adws/jsonl/__tests__/schemaFields.test.ts`: `schemaKeyFor` (system/init, assistant, no type, non-string subtype), `resolveSchemaFields` (subtype hit, bare-type fallback, miss), `extractFieldSchema` (nested object recursion, opaque `modelUsage` has no `fields`, `null` typed as `'null'`), `findMissingFields` (nested dot-path, optional absent is not missing, `null` present).
...
- Remove the old bare `result` and `system` entries. Keep field `type` values honest to the captures (`api_error_status` is `null` in a healthy `result/success`, so its type there is `"null"`; type is informational).
...
- Per line: parse (a parse error fails the fixture with the 1-based line number in `parseError`), `resolveSchemaFields`; unknown key → push `line N: type "<key>" has no schema coverage (informational)` into `extraFields`; known key → `findMissingFields`/`findExtraFields` with results prefixed `line N:` when the fixture has more than one line.
- Parser check, once per fixture: build a fresh `JsonlParserState` (same literal as today), call `parseJsonlOutput(lines.join('\n') + '\n', state)` — the trailing newline is the bug fix; then assert from the keys present in the fixture:
  ...
  - any `result/*` line → `state.lastResult !== null`;
...
- Extractor check, once per fixture: `extractor.onChunk(lines.join('\n') + '\n')`; any `assistant` → non-empty `getCurrentUsage()`; any `result/*` → `isFinalized()`.
...
- Delete `ENVELOPE_FIELDS`, `ASSISTANT_MESSAGE_ENVELOPE_FIELDS`, `mergeResultEnvelope`, `mergeUsageEnvelope`, `mergeAssistantEnvelope`. Keep `UpdateResult` and the CLI entry point.
- Unit tests in `adws/jsonl/__tests__/fixtureUpdater.test.ts`: adds a missing top-level and a missing nested required field; leaves optional fields absent; never removes an unknown field; never overwrites an existing value; multi-line fixture updated per line; no-change fixture is not rewritten (compare content and mtime).
...
- Keep `probeClaudeJsonlSchema()` exported (now returning the merged schema) and add `checkClaudeJsonlSchema(): Promise<LiveDriftReport>`; both are thin shells over `runProbe` plus the pure helpers.
...
- `result-error.jsonl` (replace content, keep name): one real `error_during_execution` line (`is_error: true`, `errors`, `terminal_reason`, no `result`), redacted.
...
- New templates: `rate-limit-event-rejected.jsonl` (the captured rejected event above with `session_id`/`uuid` set to stub ids and `unifiedWindows` dropped) and `result-rate-limited.jsonl` (`subtype: "success"`, `is_error: true`, `api_error_status: 429`, `terminal_reason: "api_error"`, `result` and the `assistant` text `You've hit your session limit · resets 2:50pm (Europe/Amsterdam)`, `stop_reason: "stop_sequence"`, zeroed `usage`, `modelUsage: {}`, `permission_denials: []`, `total_cost_usd: 0`, `num_turns: 1`, durations, `session_id`, `uuid`).
...
- New `test/mocks/stubResponse.ts` (pure): `resolveResponseMode(manifestResponse, env)` → `{ kind: 'default' } | { kind: 'rate-limited'; resetsAt: number; rateLimitType: string; limitedInvocations?: number }` with precedence manifest → `MOCK_RESPONSE=rate-limited` (+ `MOCK_RATE_LIMIT_RESETS_AT`, `MOCK_RATE_LIMIT_TYPE`) → default; `buildRateLimitedLines(eventTemplate, resultTemplate, opts)` → the three JSON strings (`rate_limit_event` with `resetsAt`/`rateLimitType` applied, an `assistant` line carrying the limit text, the `result` line) mirroring the real sequence; `shouldRateLimit(mode, invocationCount)`.
...
- `adws/jsonl/__tests__/conformanceCheck.test.ts` — temp-dir schema and fixtures: newline regression, multi-line session fixture, rejected event requires `rateLimitRejected`, missing nested required field named by dot-path, unknown subtype fallback, unknown type informational, multiple directories, and the real-fixtures smoke (`checkConformance()` with defaults passes).
- `adws/jsonl/__tests__/fixtureUpdater.test.ts` — add-only semantics per line, no delete, no overwrite, no rewrite when unchanged.
- `test/mocks/__tests__/claude-cli-stub.test.ts` — spawns the stub: rate-limited via env, via manifest marker with `limitedInvocations`, `resetsAt` override, default unchanged; output verified through `parseJsonlOutput` and `classifyProbeResult`.
...
- A `result` with an unknown subtype (`error_max_turns`) resolves to no entry and passes informationally rather than failing.
...
- `adws/jsonl/fixtures/result-error.jsonl` uses `is_error` and `session_id`, carries a real `error_during_execution` shape, and passes conformance; `result-success.jsonl` uses the snake_case names; no fixture or stub envelope contains `isError`, `durationMs`, `durationApiMs`, `numTurns` or `sessionId`.
...
- The stub emits `rate_limit_event` (with `resetsAt`) + `assistant` + `result` (`api_error_status: 429`, `is_error: true`) and exits 1 when asked through the manifest `response` block or `MOCK_RESPONSE=rate-limited`; with `limitedInvocations: 1` the second invocation is normal; the `@regression` suite reports 0 failed and the pause/resume smoke scenario keeps its baseline status.
...
- `MOCK_RESPONSE=rate-limited MOCK_STREAM_DELAY_MS=0 bun test/mocks/claude-cli-stub.ts --print --verbose --output-format stream-json ping; echo "exit=$?"` — three JSON lines (`rate_limit_event` with `"status":"rejected"` and a `resetsAt`, an `assistant` line, a `result` with `"api_error_status":429` and `"is_error":true`) then `exit=1`.
