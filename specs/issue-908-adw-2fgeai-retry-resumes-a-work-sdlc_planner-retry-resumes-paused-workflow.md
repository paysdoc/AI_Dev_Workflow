...
- **5j. Active stages:** for `starting`, `resuming`, `build_running`, `plan_running`, `stepDef_running` (dynamic phaseRunner strings): returns `false`, nothing called on any seam, `log` message matches `/running orchestrator cannot be duplicated/`.
...
### BDD seams (for the step-definition phase)

The per-issue scenarios (`features/per-issue/feature-908.feature`, tags `@adw-908 @adw-2fgeai-retry-resumes-a-work`) observe this slice only through runtime artefacts: recorded launches, `agents/paused_queue.json`, `agents/<adwId>/state.json`, the mock GitHub API's recorded comments, and captured logger output. The feature file's "Notes for the step definitions" are authoritative; in summary:

- **Cron-path rows (§1–§3) never launch a real orchestrator.** "the cron handles the ## Retry directive on issue N" calls the handler the way `trigger_cron.ts` does: `handleRetryDirective(n, [...seededComments, { body: '## Retry' }], { ...buildRetryHandlerDeps(boundary, targetRepoArgs), spawnDetached: recorder })`. `boundary` is a launch boundary for the Background's fictional target repository `acme/widgets`, and `targetRepoArgs` comes from `buildCronTargetRepoArgs`. The recorder captures script and argv through the `RetryHandlerDeps.spawnDetached` seam (the `scanAuthQueue` precedent). Top-level state names real scripts (`adws/adwChore.tsx`, `adws/adwPlanBuildReview.tsx`, or none, which must resolve to `adws/adwSdlc.tsx`), so the rows prove the script comes from `resolveResumeSpawn`. Compare scripts by repo-relative path. "targets the repository" means the recorded args carry `--target-repo acme/widgets`; `buildCronTargetRepoArgs` may append `--clone-url`, so do not assert that the args end with it.
- **Seeding.** The ADW comment is `**ADW ID:** \`<adwId>\`` with an adwId of lowercase letters, digits and hyphens only; anything else fails `extractLatestAdwId` and makes every no-op row pass vacuously. Top-level state goes through `AgentStateManager.writeTopLevelState` with the issue number, the stage, the orchestrator script when one is named, and `repoIdentity` for the target repository. The registered G20 and G6 phrases are deliberately not reused (G20 seeds a hidden adwId whose state has no issue number or script); the queue and state steps use novel phrases.
- **Pause queue.** Use the real `agents/paused_queue.json`: save and restore any pre-existing file, and delete it afterwards if it did not exist (`removeFromPauseQueue` writes `[]`). Seeded entries carry a real temporary worktree, a recording fixture orchestrator as `orchestratorScript`, and `--target-repo acme/widgets`, so a handler that wrongly resumes from the entry launches only the fixture. The scan step is `scanPauseQueue(PROBE_INTERVAL_CYCLES, () => 'clear')`. The fixture records its argv and stays alive; kill it in an `After` hook as `feature-902-queue.steps.ts` does. Launch-counting steps count recorder and fixture launches together; negative launch assertions wait a few seconds for asynchronous fixture records.
- **Comments and safety.** In an `@adw-908` `Before` hook, blank the GitHub App environment, shadow `gh` so it records `gh issue comment`, and initialise mockContext; replay the recorded calls against the mock after the When step (the `feature-902-queue.steps.ts` pattern). Assertions use G1, T14, T25 (match the resumed heading `## :arrow_forward: ADW Workflow Resuming`, not the orchestrator's `:arrows_counterclockwise:` one) and T27 (nothing posted on the host's own `paysdoc/AI_Dev_Workflow`, where #840 and #871 are real incident issues).
- **`paused_auth` log.** Capture the ADW logger's output around the handler call; the assertion matches a line naming the issue number, `paused_auth`, and the auth queue (`auth queue` / `auth-queue` / `authQueue`, any case). The Phase 2 wording satisfies it.
- **Webhook row (§4).** Drive `dispatchWebhookEvent` with an `issue_comment` payload for `acme/widgets` through a mint override whose tracker serves the issue's comments. The top-level state names the recording fixture as its orchestrator script, because this path uses the real `spawnDetached` (absolute paths pass through `resolveResumeSpawn` and `spawnDetached` unchanged). Neutralise `ensureCronProcess` by registering this process's PID for the repository (feature-821's `preventRealCronSpawn`), and save/restore any `agents/.auth_gate`. The row holds when every launch recorded for the issue carries `--target-repo acme/widgets`, including when there is none.
- **Step-definition reuse.** G18, G1, T1 and T14 are defined under `features/regression/step_definitions/`. The git/gh guard pair and T22 are defined in `feature-844.steps.ts`; do not redefine them (AmbiguousStepDefinition). G19, T25 and T27 are registered but have no step definition (feature-565 was swept); implement them to their registry semantics. Scope every hook to `@adw-908`; in `After`, remove `agents/<adwId>`, the spawn-lock artefacts, and `agents/paused_queue_logs/<adwId>.resume.log`.
- **Lock semantics have no scenario.** The held-lock no-op, the acquire → remove → spawn → release → comment ordering, and release-on-throw are pinned only by the vitest cases in steps 5f and 5g.
...
- Another paused workflow's queue entry (a different adwId) → left in place; removal is keyed by the directive's adwId, never a queue-wide clear, and nothing is launched for the other issue.
...
- `starting`, `resuming`, `install_running`, `build_running`, `test_running`, `review_running`, `document_running`, `plan_running`, `stepDef_running` → no-op; log says a running orchestrator cannot be duplicated (covers the future in-process sleeping orchestrator of #912, whose stage stays `*_running`).
...
- `## Retry` on a `paused` issue with a live queue entry removes that adwId's entry (before the spawn; every other entry stays) and spawns the orchestrator exactly once.
...
- `NODE_OPTIONS="--import tsx" bunx cucumber-js --tags "@adw-908"` — the per-issue scenarios in `features/per-issue/feature-908.feature` (§1 paused branch, §2 no-ops, §3 human-gated regression, §4 webhook caller, §5 backstops); every scenario must pass.
