...
- `features/per-issue/feature-818.feature` is written by the scenario phase before the build; read it first and implement exactly its phrases. Reuse, never redefine: the fixture-tree phrases from `feature-816.steps.ts`, `the git/gh guard runs across the whole ADW repository` / `the guard run reports no violations` (`feature-769.steps.ts`), and `the ADW TypeScript type-check passes` (`feature-504.steps.ts`).
- Adapter scenarios must be hermetic: construct the adapters in-process through their factories with a recording `runCurl` / `fetchFn` and a logger spy (the same helpers as the vitest suites); never spawn curl, never open a socket, never mutate `process.env` without restoring it.
...
Written by the scenario phase from this plan; hermetic (no `gh`, no `curl`, no network). Expected groups, mirroring `feature-816`/`feature-817`:

1. **§1 — the scope widens by exactly the two adapter packages** (AC4). Fixture-tree runs of the real guard CLI: `adws/providers/gitlab/gitlabCodeHost.ts` importing `../../core` now **fails** naming the file and citing `extraction-readiness` (the row #816 §3 pinned green, deliberately flipped); the same for `adws/providers/jira/jiraIssueTracker.ts`; a file deeper in either package (e.g. `adws/providers/jira/adf/x.ts`) importing `../../../core` fails — the entry is the directory; clean fixtures importing `../../gitContext/types`, `../../gitContext/consoleLogger`, `../types` and siblings pass; the still-out-of-scope `adws/providers/repoContext.ts` → `../core/projectConfig` and `adws/providers/github/githubCodeHost.ts` → `../../core` still pass (widening further than the issue says is a defect); both #816 seeds and both #817 entries still fail on a framework import (widen only, never narrow).
2. **§2 — configuration arrives injected, never from the environment** (AC1, AC2). Through the factories with recording seams: the GitLab argv carries the injected token and instance origin while `GITLAB_TOKEN`/`GITLAB_INSTANCE_URL` hold decoy values; the Jira request carries the injected Basic/Bearer credential while `JIRA_*` hold decoys; blank injected values are refused by name.
3. **§3 — the Logger port** (AC3). An injected logger spy receives each adapter's failure/success message with its level; with no logger injected the same line reaches `console.log` (console default preserved).
4. **§4 — ADW wiring**: `gitLabConfigFromEnv`/`jiraAuthFromEnv` over literal env values reproduce today's operator-facing messages and precedence.
5. **§5 — the ratchet**: whole-repo guard green (`Extraction-readiness scope — 6 entries`), whole-repo type-check green.
...
- `grep -nE "^export function (createGitLabCodeHost|createJiraIssueTracker)\(" adws/providers/gitlab/gitlabCodeHost.ts adws/providers/jira/jiraIssueTracker.ts | wc -l` — prints `2` (declarations pinned by the guard test's rename table).
