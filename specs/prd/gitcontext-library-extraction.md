...
- **Library repository:** created under the existing GitHub account, bootstrapped with the standard ADW init flow (project config, starter guardrail settings), registered as an ADW target. CI runs typecheck, unit tests, the ported git/gh guard (shell-out exempt set: git core and GitHub adapter; construction allowlist: `forgeProviders.ts` only; identity and extraction-readiness rules not ported), and the release job. Branch protection is enabled only after the package build issue merges, since the filtered tree has no build configuration.
