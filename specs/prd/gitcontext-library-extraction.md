...
Finish the de-tangling **inside the ADW repository**, where the full consumer base and test suite prove every shape, so that extraction is a pure file move. Then create the library repository with real history, publish `@paysdoc/gitcontext` at 1.0.0, and switch ADW over in one gated migration.
...
Extraction itself: a new ADW-registered repository seeded via history filtering with a rename map built against the post-refactor tree, a compiled ESM package with type declarations and subpath exports (core at the root, providers under a subpath), release automation whose commit parser accepts the agent-prefixed dialect, the git/gh guard ported into the library's CI with the executor as its only exempt module, and npm publishing from CI under the existing user scope. ADW then switches in one atomic human-gated migration — dependency added, imports rewritten, launch boundary repointed, extracted directories deleted, ADW's own guard left with an empty exempt set — after which version bumps arrive as Dependabot pull requests merged by hand.
...
14. As a library consumer, I want one package with the core at the root import and the provider layer under a subpath, so that a consumer needing only git operations never touches forge machinery.
...
22. As a library maintainer, I want the git/gh guard ported into the library's CI with the executor module as its only exemption, so that agents modifying the library cannot erode the spawn chokepoint.
...
- **Package:** single npm package under the existing user scope, MIT licensed, compiled ESM plus type declarations, exports map with the core at the package root and providers under a subpath. First version 1.0.0.
...
- **Library repository:** created under the existing GitHub account, bootstrapped with the standard ADW init flow (project config, starter guardrail settings), registered as an ADW target. CI runs typecheck, unit tests, the ported git/gh guard (exempt set: the executor module only), and the release job.
