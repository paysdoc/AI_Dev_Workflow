...
- **Layer 1 regression fixture (follow-up to Step 8).** The captured Cucumber `checkInstall` stack trace from `/private/tmp/claude-501/.../tasks/bos7x8k2w.output` should be dropped into the in-repo regression suite to lock in "syntax check must not execute step files". The issue body explicitly identifies this fixture as a test asset for the prompt fix, and the BDD scope notes in `features/per-issue/feature-521.feature` reference it as part of how Layer 1 is verified outside the BDD layer. Not included as a Step here; tracked as a Layer 1 follow-up so the prompt-edit (Step 8) is the only Layer 1 task in this plan.
- **Layer 3 follow-up (out of scope here).** File a minimal repro of "Bash tool subprocess outlives `result: success`" against `claude-cli` as a separate upstream issue. This plan does not include the upstream report — it stands on layers 1 and 2 alone, either of which would have prevented the incident.
