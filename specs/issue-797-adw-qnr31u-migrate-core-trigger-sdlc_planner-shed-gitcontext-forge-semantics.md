...
- **`trigger_cron.ts`** — `fetchOpenIssues()` moves out of the module as the exported `listCronOpenIssues(issueTracker)` in the new `adws/triggers/cronIssueListing.ts`, and the cron calls it with `cronBoundary.providers.issueTracker`; the listing itself is `issueTracker.listIssues({ fields: ['number','title','body','comments','createdAt','updatedAt','labels'], limit: 100 })` with the same try/catch → `[]` policy. The extraction is what makes the listing reachable: `trigger_cron.ts` has import-time side effects (`setInterval`, the entry-script guard, the boundary build), so a module-private `fetchOpenIssues` cannot be driven from a step definition — the same finding #796 recorded for `initializeWorkflow`, and it keeps this file (already over the line guideline) from gaining a function. `buildTargetRepoArgs()` reads `cronGitContext.remoteUrl()` (a git op on the context it already holds) instead of constructing a second context; `runUpgradeRedriveScan` receives deps built from `cronBoundary.providers.codeHost`; both sweep ticks pass `{ boundary: cronBoundary }`. `checkAndTrigger` gains one guard at its top — no boundary (only possible when the module is imported, where the tick never runs) logs and returns. After this the file constructs nothing: its `gitContextForRepo` import goes, and its transitional guard entry is deleted by the ratchet.
...
- **`remoteReconcile.ts`** — named in the issue's Touched Files, and its `findPRByBranch` is a forge read: `buildDefaultReconcileDeps(boundary)` replaces the no-argument builder for every caller that holds a boundary (the no-`deps` legacy path keeps today's `repoInfo` wiring, Task 12); `findPRByBranch` → `boundary.providers.codeHost.findPullRequestByBranch(branchName)` and `branchExistsOnRemote` → `boundary.gitContext.lsRemote(branchName)` (a git op, moved off its own `gitContextForRepo` construction onto the boundary's context). `ReconcileDeps.findPRByBranch` narrows to `(branchName) => Pick<PullRequestSummary, 'state'> | null` and `mapArtifactsToStage` reads only `.state`, so the OPEN → `awaiting_merge` / MERGED → `completed` / CLOSED → `discarded` mapping is unchanged — `findPullRequestByBranch` is state-agnostic (it wraps the same `defaultFindPRByBranch`), so PRs that are no longer open stay visible and a finished run does not read as unfinished. `deriveStageFromRemote(issueNumber, adwId, repoInfo, deps?)` keeps `deps` optional and falls back to a module-private `repoInfo` wiring identical to today's, so the two legacy takeover callers that hold no boundary (`scanAuthQueue`, `webhookGatekeeper`) are byte-unchanged. **No memoisation anywhere on this path**: the mandatory re-verification read must reach `codeHost.findPullRequestByBranch` a second time, or the read-your-write cross-check the module exists for degenerates into ceremony.
- **`takeoverHandler.resolveAdwId`** — the trigger half of the issue's "comment handling": `buildDefaultTakeoverDeps(repoInfo?, boundary?)` reads `boundary.providers.issueTracker.fetchComments(issueNumber)` when a boundary is in hand, mapping to `{ body }[]` for the unchanged `extractLatestAdwId` (which takes the **latest** match, so the tracker's chronological comment order is load-bearing), and builds its `deriveStageFromRemote` dep as `buildDefaultReconcileDeps(boundary)`. `EvaluateCandidateInput` gains an optional `boundary?: LaunchBoundary` beside the `gitContext?` it already carries for the same reason ("absent only in legacy callers"); the cron passes `cronBoundary`. Without a boundary (`scanAuthQueue`, `webhookGatekeeper`) both deps fall back to today's wiring — `issueListApi.fetchIssueCommentBodies` (§5) and the `repoInfo` reconcile deps — so the one command-shape change is confined to the cron's takeover path (see Notes).
...
`concurrencyGuard.ts`, `webhookGatekeeper.closeAbandonedDependents` and `issueClosedUnblockRouter.buildDefaultDependencyUnblockDeps` sit at the bottom of `repoInfo`-parameterised call chains (`checkIssueEligibility` → `isConcurrencyLimitReached`; `handleIssueClosedEvent` → `closeAbandonedDependents`/`handleIssueClosedDependencyUnblock`) reached from both the cron and the webhook server, and no provider is in hand there. Threading a boundary through that graph is a signature change to five exported functions and their suites for zero AC movement — the same trade #796 §7 made for `adwChore`/`adwClearComments`/`unitTestPhase`. These three therefore switch to a new thin wrapper module, exactly as their siblings already use `fetchLinkedPRs(repoInfo)`, `closeIssue(…, repoInfo)` and `commentOnIssue(…, repoInfo)`. `takeoverHandler.resolveAdwId` does **not** join them — `evaluateCandidate`'s cron caller already holds the boundary, so it takes the providers route of §4 and keeps the wrapper only as its no-providers fallback:
...
`fetchIssueCommentBodies` keeps `takeoverHandler`'s current `gh issue view … --json comments --jq '.comments'` command verbatim, so the no-providers takeover callers (`scanAuthQueue`, `webhookGatekeeper`) emit exactly the command they emit today. After this, `concurrencyGuard.ts`, `webhookGatekeeper.ts` and `issueClosedUnblockRouter.ts` construct no context at all (their guard entries are deleted by the ratchet); `takeoverHandler.ts` keeps `gitContextForSync` for its git ops only. The pure `cronIssueFilter.ts` and `upgradeFailureCap.ts` are left untouched (the "upgradeFailureCap consumer" is `adwUpgrade.tsx`, migrated in #796) — and `adwUpgrade`'s comment read must stay on `providers.issueTracker.fetchComments`, whose REST route populates `IssueComment.author`: `isUpgradeFailureComment` gates on `author.endsWith('[bot]')`, so "tidying" it onto `fetchIssue(n).comments` would zero the failure count and silently disable upgrade escalation.
...
### Triggers
- `adws/triggers/trigger_cron.ts` — `fetchOpenIssues` (`:94-105`, extracted to `cronIssueListing.ts`), `buildTargetRepoArgs` (`:108-114`), `boundPerIssueSweep`/`boundPromotionSweep` (`:148-157`), the redrive call (`:350-354`), `evaluateCandidate` input (`:407`, gains `boundary`), the `gitContextForRepo` import (`:42`).
...
- `adws/triggers/concurrencyGuard.ts:20-30`, `adws/triggers/webhookGatekeeper.ts:198-231` (+ import `:30`), `adws/triggers/issueClosedUnblockRouter.ts:50-58` (+ import `:19`).
- `adws/triggers/takeoverHandler.ts:55-61,79,86-94,176-182` (+ import `:33`) — `EvaluateCandidateInput.providers`, `buildDefaultTakeoverDeps(repoInfo?, providers?)`, `resolveAdwId` via the tracker with the wrapper fallback, `deriveStageFromRemote` deps built from the boundary.
...
- `adws/core/remoteReconcile.ts:26-30,47-52,60-66,90-105` (+ imports `:17-19`) — `ReconcileDeps.findPRByBranch` narrows to a `{ state }` reader, `buildDefaultReconcileDeps(boundary)`, `deps` required on `deriveStageFromRemote`; `adws/core/index.ts:207` re-export unchanged.
...
- `adws/triggers/cronIssueListing.ts` — the exported `listCronOpenIssues(issueTracker)` extracted from `trigger_cron.fetchOpenIssues`, plus the `RawIssue` shape `cronIssueFilter` consumes (≈30 lines).
- `adws/triggers/__tests__/cronIssueListing.test.ts`.
...
Walk the callers over, innermost first so each step compiles: the `adws/github/*` layer and `GitHubCodeHost` onto `createGhRepoApi`; the three `repoInfo`-only trigger helpers onto `issueListApi`; the cron (via the extracted `cronIssueListing`), sweeps, redrive, takeover and pause-queue scanner onto boundary providers; the phases/core utilities (`workflowInit`, `prReviewPhase`, `upgradeGate`/`upgradeClaim`, `targetRepoManager`, `remoteReconcile`, health check) onto `bindWorkspaceContext`, `buildDefaultReconcileDeps(boundary)` and provider thunks; delete the dead `postWorkflowComment`/`postPRWorkflowComment` and `vcs.getDefaultBranch`.
...
### 8. Move the three `repoInfo`-only trigger helpers onto `issueListApi`
...
- `takeoverHandler.buildDefaultTakeoverDeps(repoInfo?, boundary?)`: `EvaluateCandidateInput` gains `boundary?: LaunchBoundary` (documented like the existing `gitContext?`: "absent only in legacy callers") and `evaluateCandidate` passes `input.boundary` to the default deps builder. `resolveAdwId` becomes, inside the existing try/catch → `null`: `boundary ? extractLatestAdwId(boundary.providers.issueTracker.fetchComments(issueNumber).map((c) => ({ body: c.body }))) : extractLatestAdwId(fetchIssueCommentBodies(issueNumber, repoInfo))`. Leave the `deriveStageFromRemote` dep on its current no-`deps` call for now; Task 12 gives it `boundary ? buildDefaultReconcileDeps(boundary) : undefined` once that builder exists. Drop the `gitContextForRepo` import (keep `gitContextForSync`).
- Tests: in the three wrapper suites replace the `gitContextForRepo` mock with a mock of `../../github/issueListApi` (`listIssues`), keeping every assertion on inputs/outputs; in `takeoverHandler.test.ts:790-822` keep the existing no-providers cases against `fetchIssueCommentBodies` (called with `(99, REPO)`; a throw / unparseable payload yields `null`) and add a providers case asserting the tracker's comments are read and the **latest** adw id wins.
- Suite green.
...
- Create `adws/triggers/cronIssueListing.ts`: move the `RawIssue` interface out of `trigger_cron.ts` and export `listCronOpenIssues(issueTracker: Pick<IssueTracker, 'listIssues'>): RawIssue[]` = `issueTracker.listIssues({ fields: ['number','title','body','comments','createdAt','updatedAt','labels'], limit: 100 }) as RawIssue[]` wrapped in the cron's existing `try/catch` → `log('Failed to fetch issues: …', 'error')` + `[]`. The field list, the log text and the swallow policy are byte-identical to today's; the module has no import-time side effects, so cucumber and vitest can both reach it (`trigger_cron.ts` cannot be imported — `setInterval`, the entry-script guard and the boundary build all fire on import).
- `trigger_cron.ts`: (a) `checkAndTrigger` starts with `const boundary = cronBoundary; if (!boundary) { log('checkAndTrigger: no launch boundary (module imported, not launched) — skipping tick', 'warn'); return; }` and uses `boundary.gitContext`/`boundary.providers` below; (b) delete the private `fetchOpenIssues` and call `listCronOpenIssues(boundary.providers.issueTracker)`; (c) `buildTargetRepoArgs()` → `() => { try { return cronGitContext?.remoteUrl() ?? null; } catch { return null; } }`; (d) `runUpgradeRedriveScan(issues, repoInfo, targetRepoArgs, buildDefaultUpgradeRedriveDeps(repoInfo, boundary.providers.codeHost))`; (e) `boundPerIssueSweep`/`boundPromotionSweep` close over `cronBoundary` and pass `{ boundary }`; (f) `evaluateCandidate({ …, gitContext: boundary.gitContext, providers: boundary.providers })`; (g) delete the `gitContextForRepo` import. Do not add a function to this file (it is already over the line guideline) — the listing lives in `cronIssueListing.ts` for exactly that reason.
- Tests: `cronIssueListing.test.ts` (the exact seven-field query reaches `listIssues`; the parsed entries keep `body`/`comments`/`labels`/`createdAt`/`updatedAt` so `cronIssueFilter.evaluateIssue` still reads them; a throwing tracker logs and yields `[]`), `upgradeRedrive.test.ts` (deps shape), `promotionSweepDefaults.test.ts` (`makeDefaultDeps({ gitContext: fakeCtx, repoId, providers: { issueTracker, codeHost } })`; assert `listIssues` query, `getDefaultBranch`, `createIssue` + `applyLabel` calls), `perIssueScenarioSweep.test.ts` (`boundary` fakes; the "default getMergedAt routes through the injected context" case becomes "…through `boundary.providers.codeHost.listMergedPullRequests` (never `gitContextForRepo`)"), `perIssueSweepPersist.test.ts` (`openPr` returns a number; `findOpenSweepPr`/`mergePr` via `codeHost` doubles), and the cron tick tests if any construct sweeps with `gitContext`.
- Suite green.
...
### 12. Core utilities: remote reconcile, claim deps, workspace manager, dead code, health check

- `remoteReconcile.ts`: `ReconcileDeps.findPRByBranch: (branchName: string) => Pick<PullRequestSummary, 'state'> | null` and `branchExistsOnRemote: (branchName: string) => boolean` (both lose their `repoInfo` parameter — the identity is closed over by whichever builder made them); `readOnce` and `deriveStageFromRemote` drop the `repoInfo` pass-through into deps. Add `buildDefaultReconcileDeps(boundary: LaunchBoundary)` — `readTopLevelState` unchanged, `branchExistsOnRemote: (b) => { try { return boundary.gitContext.lsRemote(b).length > 0; } catch (err) { log(…same warn text…); return false; } }`, `findPRByBranch: (b) => boundary.providers.codeHost.findPullRequestByBranch(b)` — and keep a module-private `repoInfo` wiring (`defaultFindPRByBranch(b, repoInfo)` + `gitContextForRepo(repoInfo).lsRemote(b)`, exactly today's) that `deriveStageFromRemote(issueNumber, adwId, repoInfo, deps?)` falls back to when `deps` is omitted, so `scanAuthQueue`/`webhookGatekeeper` are untouched. `mapArtifactsToStage` keeps its exported signature but reads `Pick<PullRequestSummary, 'state'> | null` — the OPEN/MERGED/CLOSED switch is untouched. Add nothing that caches: both reads inside the verification loop must reach the code host.
- `takeoverHandler.ts` (finishing Task 8): the `deriveStageFromRemote` dep becomes `(n, adwId, r) => deriveStageFromRemote(n, adwId, r, boundary ? buildDefaultReconcileDeps(boundary) : undefined)`.
- Tests: `remoteReconcile.test.ts` — the deps doubles lose their `repoInfo` argument; add (a) an outline over OPEN → `awaiting_merge`, MERGED → `completed`, CLOSED → `discarded` through a `codeHost` double reached via `buildDefaultReconcileDeps(boundary)`, and (b) a case asserting `findPullRequestByBranch` is called at least twice for one `deriveStageFromRemote` (the read-your-write cross-check is not memoised); keep the existing no-`deps` cases as the legacy-path proof.
...
- Create `features/per-issue/step_definitions/feature-797.steps.ts` for every step phrase in `features/per-issue/feature-797.feature`, reusing `gitContextSharedWorld.ts` (`makeSpyExec`, `makeFullOptions`, `makeNoOpFsDeps`, `FRAMEWORK_ROOT`) and, for "the git context was asked for no forge-semantic operation" steps, the Proxy watcher pattern from `feature-796.steps.ts:224-260`. Build this file's **own** recording-provider world: the Given reads `the repository {string} is launched with recording providers` and must not reuse `feature-796.steps.ts:501`'s near-identical boundary phrase — 796's stand-ins record calls but answer none of the listings, merged-PR lookups or default-branch reads §2–§6 depend on. Every scenario uses the deliberately non-existent `adw-fixture/void-797`; do not substitute a real repository.
- Row by row: §1 copies `adws/gitContext/` (minus `__tests__`) into an `mkdtempSync` directory and dynamically imports the copied `index.ts`, asserting the load outcome (removed in `After`), then probes a constructed context's member names; §2 drives `listCronOpenIssues(boundary.providers.issueTracker)` and feeds the result to `cronIssueFilter.evaluateIssue`; §3 drives `runPromotionSweep`'s `makeDefaultDeps`, `defaultGetMergedAt` and `prepareSweepBase` from the boundary; §4 drives `deriveStageFromRemote` over `buildDefaultReconcileDeps(boundary)`; §5 drives `postIssueStageComment(boundary.providers, …)` and `buildDefaultTakeoverDeps(repoInfo, boundary).resolveAdwId`; §6 drives `ensureTargetRepoWorkspace` over an `mkdtempSync` `targetReposDir`; §7 builds `new GitContext(opts, { exec })` and runs `createGhRepoApi(ctx).listOpenIssues(...)` over it; §8 shells the guard and the type-check through the reused phrases.
- Reuse, never redefine (an AmbiguousStepDefinition otherwise): `the ADW codebase is checked out` (`features/step_definitions/ensureCronOnEveryEventSteps.ts:8`), `the ADW TypeScript type-check passes` (`feature-504.steps.ts:1126`), `the git/gh guard is run across the repository` and `the git/gh guard reports no violations` (`feature-691.steps.ts:82,91`).
...
- **Triggers** — concurrency guard / gatekeeper / unblock router on mocked `issueListApi` with unchanged input/output assertions; takeover `resolveAdwId` on both routes (no boundary → `fetchIssueCommentBodies(99, REPO)`, boundary → the tracker's comments, latest adw id wins, throw → `null`); `listCronOpenIssues` field list, field survival into `cronIssueFilter`, and `[]`-on-throw; `buildTargetRepoArgs` reads `remoteUrl` from the launch context and yields `null` on throw; redrive deps built from a `codeHost` double; promotion defaults (`listIssues` query, `getDefaultBranch`, `createIssue` then `applyLabel` per label, branch-guard no-op); per-issue sweep (`listMergedPullRequests(200)`, `null` on throw, stale/exempt decisions unchanged); persist (`openPr` returns the number, duplicate-PR skip, merge failure left open); pause-queue scanner (four comment paths post through `fakeBoundary.providers`; throwing boundary swallowed; target-repo entry builds a target boundary).
- **Phases/core** — `workflowInit` calls `bindWorkspaceContext` once with the boundary's identity and not at all on refusal; `upgradeGate` deps shape; `upgradeClaim` with explicit `getDefaultBranch`; health-check probes through the adapter; `remoteReconcile` over `buildDefaultReconcileDeps(boundary)` — OPEN/MERGED/CLOSED all map (a no-longer-open PR must not read as "no PR"), the verification read hits the code host a second time, and the no-`deps` legacy path is unchanged.
...
- `trigger_cron.ts` imported as a module (BDD): no boundary → `checkAndTrigger` returns early; `listCronOpenIssues` is never reached; sweep ticks still take the `sweep === null` skip.
...
- `ensureTargetRepoWorkspace` against a workspace that is **not** on disk: `ensureRepoWorkspace` takes the clone branch and never invokes `getDefaultBranch` at all (`repoWorkspace.ts:119-122`), so no provider is minted and no `.adw/providers.md` is read from the missing directory; the default-branch read happens only on the already-cloned fetch branch, where the workspace exists by construction. Do not "fix" this by reading the branch before the clone.
- `deriveStageFromRemote` called without `deps` (legacy `scanAuthQueue`/`webhookGatekeeper` takeover path): unchanged `repoInfo` wiring, unchanged commands.
...
- `IssueTracker.listIssues` reaches the cron through the exported `listCronOpenIssues`, and `CodeHost.findPullRequestByBranch` reaches `remoteReconcile` through `buildDefaultReconcileDeps(boundary)`; neither path memoises, so the reconcile's verification read hits the forge a second time.
- Operator-visible behaviour is unchanged: identical gh commands per operation, same repositories, same comment bodies and log lines — with two documented exceptions: target-repo paused entries now receive the error comments the old cwd-remote check silently dropped, and the cron's takeover `resolveAdwId` reads comments over the tracker's REST route instead of `gh issue view` (same bodies, same order).
...
- **The one command-shape change, and why it is accepted.** On the cron's takeover path only, `resolveAdwId`'s comment read moves from `gh issue view … --json comments --jq '.comments'` to the tracker's REST `fetchIssueCommentsRest` (`gh api …/comments --paginate`). AC5's "same commands" pulls one way; the issue's "trigger comment handling migrates to providers" and `feature-797.feature` §5 (which drives `buildDefaultTakeoverDeps` from the boundary and asserts the **latest** adw id wins) pull the other. Same repository, same comment bodies, same chronological order, same `null`-on-failure policy — only the transport differs, and the no-boundary callers keep the old command verbatim through `fetchIssueCommentBodies`. Call it out in the PR body beside the pause-queue fix.
