...
- **Tests**: unit tests for `selectPreferredPR`, each new `adwMerge` exit, the `merge_blocked` cron ineligibility, `isRetryComment` parsing, and `handleRetryDirective` reset; plus the per-issue BDD scenarios in `features/per-issue/feature-527.feature` (tagged `@adw-527`), whose §5 covers the `merge_blocked → ## Retry → merged` round-trip.
...
- `features/per-issue/step_definitions/feature-527.steps.ts` — per-issue step definitions for the **already-on-disk** `features/per-issue/feature-527.feature` (tagged `@adw-527`), mirroring `features/per-issue/step_definitions/feature-524.steps.ts`. Reuses the shared regression steps (`G1`/`G4`/`G6`/`G8`/`G10`/`G11`/`W1`/`W10`/`T1`/`T2`/`T3`/`T5`/`T7`) and defines the feature-527-specific phrases (see Task 13). The `.feature` file itself is **not** a new file — it already exists; state is seeded via `G6`/`G11`, so no claude-cli-stub manifest is required.
...
### Task 13 — Author the step definitions for the existing `@adw-527` BDD scenarios (incl. the `merge_blocked → ## Retry → merged` round-trip)
The BDD coverage for this bug **already exists on disk**: `features/per-issue/feature-527.feature` (tagged `@adw-527 @adw-22y8n3-adwmerge-dead-ends-i`), §1–§7, including the `merge_blocked → ## Retry → merged` round-trip in §5. **Do not author a new `.feature` file** — in particular, do **not** create `features/regression/smoke/merge_blocked_retry.feature`; the per-issue scenarios supersede it. The work here is to create the step-definition file that makes those scenarios runnable, mirroring the per-issue pattern in `features/per-issue/step_definitions/feature-524.steps.ts`.
- Create `features/per-issue/step_definitions/feature-527.steps.ts` (auto-loaded via `cucumber.js` → `features/per-issue/step_definitions/**/*.ts`). Add `Before`/`After` hooks tagged `@adw-527` that call `setupMockInfrastructure`/`teardownMockInfrastructure` and reset per-scenario state (model on `feature-524.steps.ts`). Track the most-recently-seeded `adwId` in module scope so the `## Retry` When step can resolve it.
- **Reuse the shared regression steps** (already defined in `features/regression/step_definitions/{givenSteps,whenSteps,thenSteps}.ts`; do not redefine): `G1` (accept issue comments), `G4` (issue exists), `G6` (state file at stage), `G8` (record PR-list calls), `G11` (worktree init at branch), `W1` (invoke `{string}` orchestrator), `W10` (cron probe runs once), `T1` (records workflowStage), `T2` (recorded a comment on issue), `T3` (recorded a comment containing text), `T5` (subprocess exited), `T7` (zero PR-merge calls). Confirm each phrase string matches the feature file verbatim before relying on it; define any that do not yet exist. (Note: the per-issue scenarios reach `completed` via a real open-PR merge, so `G10` "return PR as merged" is **not** used here.)
- **Define the new feature-527-specific phrases** (all assert against artefacts only — mock-recorded GitHub calls + the top-level state file via `AgentStateManager.readTopLevelState` — never source files, per the `vocabulary.md` rot rubric):
  - *Given (PR fixtures on the mock):* `the branch {string} carries a closed PR {int} and an open PR {int}`; `the branch {string} carries open PR {int} and a more recent open PR {int}`; `the branch {string} has no pull request`; `the branch {string} carries a single open PR {int}`; `the branch {string} carries a single closed unmerged PR {int}`; `the branch {string} carries a single open unapproved PR {int}`; `automatic conflict resolution for PR {int} fails after the maximum attempts`.
  - *Given (state / issue setup):* `the state file for adwId {string} is seeded with a merge retry count of {int}` — writes `mergeRetryCount` into the **top-level** state (the counter analogue of `G6`'s stage seed; kept textually distinct from the Then assert below so no single Cucumber step both writes and asserts the counter); `issue {int} carries an ADW comment naming adwId {string}` — posts an `**ADW ID:** \`<adwId>\`` comment to the mock so the cron's `extractLatestAdwId` resolves it; `issue {int} has a comment whose body is {string}`; `issue {int} does not carry the {string} label`.
  - *When:* `the {string} directive is processed for issue {int}` — for the `"## Retry"` directive (phase-import pattern), import `handleRetryDirective` from `adws/triggers/retryHandler` and call `handleRetryDirective(issueNumber, [{ body: '**ADW ID:** \`<adwId>\`' }])`, where `<adwId>` is the adwId seeded for this scenario (tracked from the preceding `G6` / seed step). Assertion target = top-level state file artefact.
  - *Then:* `the state file for adwId {string} records a merge retry count of {int}` — reads `mergeRetryCount` (the assert analogue of the seed above); `the mock GitHub API recorded a PR-merge call for PR {int}`; `the mock harness recorded no PR-merge call for PR {int}`; `the merge-blocked escalation comment on issue {int} names the blocking cause` and `the merge-blocked escalation comment for issue {int} names the blocking cause` (both forms appear in the file — assert a recorded comment whose body contains `Merge Blocked` and a cause line); `the mock GitHub API recorded a comment on issue {int} containing the text {string}`; `no orchestrator is spawned for issue {int}`; `no merge orchestrator is dispatched for issue {int}`; `the merge orchestrator is dispatched for issue {int}` (assert against the cron probe's recorded dispatch/spawn requests from `W10`).
- Document each new phrase in `features/regression/vocabulary.md` at the next free `G#`/`W#`/`T#` IDs, with the execution pattern (`mock-query` for recorded-call assertions, `subprocess` for the orchestrator/cron-driven assertions, `phase-import` for the `## Retry` directive step) and assertion target, exactly as the existing rows do.
- The §5 round-trip reaches `completed` through the **real merge path** (the branch carries a single open PR → `W1` invokes `adwMerge` → the open PR is merged → `executeMerge` writes `completed`), not via an `already_merged` shortcut. The §1 scenarios already exercise the same recorded-PR-merge channel, so no new merge-execution mock is required beyond what those steps establish.
- Ensure the seed/assert steps target the **top-level** state file that `handleRetryDirective` and `adwMerge` read (`AgentStateManager.writeTopLevelState`/`readTopLevelState`) — the same artefact `T1` reads — not a worktree-local `.adw/state.json`. Verify against the existing `G6`/`T1` implementations and reuse that path. Follow the rot-prevention rule in `vocabulary.md`: assert only state-file / recorded-call **artefacts**, never source files.
...
- `NODE_OPTIONS="--import tsx" bunx cucumber-js --tags "@adw-527"` — the per-issue BDD scenarios (`features/per-issue/feature-527.feature`, §1–§7 including the `merge_blocked → ## Retry → merged` round-trip) all pass.
- `NODE_OPTIONS="--import tsx" bunx cucumber-js --tags "@regression"` — the permanent regression suite still passes with zero regressions.
- Targeted scenario check: `NODE_OPTIONS="--import tsx" bunx cucumber-js features/per-issue/feature-527.feature` — the issue-527 scenarios (incl. the round-trip) pass.
