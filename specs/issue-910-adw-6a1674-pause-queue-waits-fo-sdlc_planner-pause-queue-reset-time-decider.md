...
- **Decider:** `adws/triggers/pauseQueueDecider.ts` exports `isBeforeReset(entry, now)`, `decidePauseQueueAction(input)` and the `PauseQueueAction` discriminated union. It imports only types and the ISO helper; no `../core`, no clock, no logging. Decision table (top to bottom):
  ...
  | absent, unparseable, or past   | `limited`           | `refresh_reset` carrying the probe's reset time (and limit type) if it reports a reset time; bare otherwise |
  ...
  `refresh_reset` is the one action for a confirmed limit. When the probe reports a reset time, the action carries it (converted to ISO) together with the probe's limit type, if any. The two describe the same limit, so they are refreshed as a pair. When the probe reports no reset time, the action is bare, even if the probe named a limit type, and the shell writes only `lastProbeAt`. That is the acceptance criterion's "`limited` without one → no strike, no change": no strike, and no change to the reset time or the limit type (the #910 scenario "A "limited" classification with no reset time counts no strike and changes nothing").
...
- `features/per-issue/step_definitions/feature-902-queue.steps.ts` — drives the real `scanPauseQueue(PROBE_INTERVAL_CYCLES * i, () => probeRateLimit(probeStub.exec))` over the real queue file with legacy-shaped seeded entries (no `resetsAt`). Must keep compiling and passing (the `@adw-902` scenarios are the live legacy-path proof). G20 (`seedPausedWorkflow`) writes top-level state `{ adwId, workflowStage: 'paused' }`. Its `Before`/`After` hooks (≈119, ≈141) are tagged `'@adw-902 or @adw-907'` and are widened to include `@adw-910` (step 8).
- `features/per-issue/step_definitions/feature-902.steps.ts` — the probe stub behind "the Claude CLI answers the rate-limit probe with exit code {int} and {word}:" and a `Before` hook (≈62) tagged `'@adw-902 or @adw-907'`, widened to include `@adw-910` (step 8).
- `features/per-issue/feature-902.feature` — amended for #910 (see its "AMENDED BY #910" note). Four §3 rows carry `@adw-910`: the three-hour journey, the threshold row, the 2026-09-24 replay, and the unknown-drop row (≈339), renamed "A genuinely unknown probe failure still counts, and the third one drops the workflow with a comment that names ## Retry as the recovery". That row asserts the eviction comment contains "failed to resume after 3 probe attempts" (≈350) and "## Retry" (≈351). The new message keeps the first phrase and adds the second.
- `features/per-issue/feature-907.feature` — the expired-login row (≈252) carries `@adw-910`. A `failed` verdict counts a strike at every probe, and the third evicts with a comment.
- `features/per-issue/feature-910.feature` — this issue's scenarios: §1 the pause path writes the reset facts; §2 the decider's decision tables; §3 the scanner's reset-time gate, the #840 replay and the end-to-end journey; §4 a `limited` probe refreshes the reset time and never strikes; §5 eviction leaves the stage `paused` and names `## Retry`; §6 legacy entries; §7 type-check and git/gh guard backstops. Its "Notes for the step definitions" are the harness contract (step 8).
...
- `export function decidePauseQueueAction(input: PauseQueueDecisionInput): PauseQueueAction`:
  ...
  - `refreshAction(probe)`: when `probe.resetsAt` is a finite number, `{ kind: 'refresh_reset', resetsAt: resetsAtIsoFromEpochSeconds(probe.resetsAt) }` plus `rateLimitType` when it is a string. Otherwise the bare `{ kind: 'refresh_reset' }`, even when the probe named a limit type: a `limited` probe without a reset time changes nothing on the entry (the issue's "`limited` without one → no strike, no change"), so the stored reset time and limit type always describe the same limit. Absent facts are absent keys. No comparison with the entry's current value: an identical reset time is an idempotent refresh, and `Date` math on the entry is not the decider's business here.
...
- Refresh: `limited` with `{ rateLimitType: INCIDENT_RATE_LIMIT_TYPE, resetsAt: INCIDENT_RESETS_AT }` after a past reset → `{ kind: 'refresh_reset', resetsAt: BEFORE, rateLimitType: 'five_hour' }` (`toEqual`, so no stray keys); `limited` with `{ rateLimitType: 'seven_day' }` only → `{ kind: 'refresh_reset' }` exactly (no reported reset time means no change, not even to the limit type); `limited` with no facts → `{ kind: 'refresh_reset' }` exactly; `limited` with no facts and `probeFailures: 2` → still `refresh_reset` (one short of the budget, never evicted); `limited` on a legacy entry with facts → refresh (a legacy entry gains a reset time from the first probe that reports one).
...
- New `scanPauseQueue` cases:
  ...
  - **a limited probe that names a limit type but reports no reset time changes nothing but `lastProbeAt`** (feature-902's three-hour journey as a unit test): legacy entry; probe `{ verdict: 'limited', rateLimitType: INCIDENT_RATE_LIMIT_TYPE }`; `updatePauseQueueEntry(adwId, { lastProbeAt: NOW.toISOString() })` with exactly that object, so no `rateLimitType`, `resetsAt` or `probeFailures` key.
...
- `features/per-issue/step_definitions/feature-902-queue.steps.ts` and `feature-908.steps.ts` (after step 1): no logic change to the existing steps; confirm `bun run test` (tsc covers `features/**`) passes with the two-argument `scanPauseQueue` calls. G20's seeded entries have no `resetsAt`, so the `@adw-902` scenarios remain the live legacy-path proof; run them. Four of them also carry `@adw-910`: the three-hour journey, the threshold row, the 2026-09-24 replay, and the unknown-drop row. The unknown-drop row is renamed "A genuinely unknown probe failure still counts, and the third one drops the workflow with a comment that names ## Retry as the recovery" and asserts both "failed to resume after 3 probe attempts" and "## Retry"; the step 6 eviction message satisfies both. `feature-907.feature`'s expired-login row (a `failed` verdict strikes, and the third evicts with a comment) also carries `@adw-910`.
- `features/per-issue/feature-910.feature` exists (§1–§7). It reuses G1, G18, G20, T2, T3, T14 and T22 and the #902/#907/#844 phrases. Its novel phrases get step definitions after the build:
  - the clock: "the cron host's clock reads {string}";
  - the reset facts on a seeded entry: "the paused workflow for issue {int} was queued with a {string} limit that resets at {string}";
  - the legacy-shaped seed: "a workflow for issue {int} was paused in the rate-limit queue for the target repository {string} by a release that recorded no reset time or limit type";
  - the pause path: "a workflow for issue {int} is running its {string} phase for the target repository {string}" and "the {string} phase is stopped by a rate-limit error carrying …" (a limit with a reset time, a limit with no reset time, or no facts);
  - how often the scanner probed: "the scanner did not run the rate-limit probe" and "the scanner ran the rate-limit probe once per probe cycle";
  - entry assertions: "the pause queue entry for issue {int} records the reset time {string}", "… records the limit type {string}", "… records no reset time", "… records no limit type" and "… stores its reset time as an ISO 8601 timestamp";
  - the stage: "the workflow for issue {int} is recorded at workflow stage {string}";
  - the decider: "a pause-queue entry with …", "the rate-limit probe classification is …", "the pause-queue decider is consulted at {string}", "the pause-queue decider returns …", "… neither resumes, strikes nor evicts the entry" and "… sets no new reset time on the entry".

  The feature file's "Notes for the step definitions" are the full harness contract. The points that depend on this plan:
  - Widen the existing hook tags from `'@adw-902 or @adw-907'` to `'@adw-902 or @adw-907 or @adw-910'`: the `Before` in `feature-902.steps.ts` (≈62) and the `Before`/`After` in `feature-902-queue.steps.ts` (≈119, ≈141). Do not register second hooks; a second hook would initialise the mock infrastructure twice (the #907 lesson).
  - The clock is pinned through `scanPauseQueue`'s third argument (`{ now: () => pinnedDate }`). The shared "the pause-queue scanner runs N probe cycle(s)" step passes it when a clock is pinned and omits it otherwise. Never fake `setTimeout`/`setInterval`.
  - The decider steps call `decidePauseQueueAction({ entry, probe, now, maxProbeFailures: MAX_UNKNOWN_PROBE_FAILURES })` in process. This slice has no repo-identity input, so none is passed.
  - The pause-path steps drive the real `runPhase` with a phase function that throws `new RateLimitError(phase, facts)` (`resetsAt` in epoch seconds). `process.exit` is replaced by a sentinel-throwing stub for the duration of the call. The entry names the real orchestrator script, so the end-to-end row shadows `bunx` on `PATH` to intercept the relaunch.
  - G20 already writes top-level state `{ adwId, workflowStage: 'paused' }`. The §5 assertion that the stage is still "paused" after eviction relies on the scanner writing no state.
...
- `adws/triggers/__tests__/pauseQueueDecider.test.ts` — new, mock-free: `parseResetsAt`/`isBeforeReset` boundaries; the reset gate over all four verdicts; resume; refresh with full facts, with a limit type alone (bare, so no change) and with no facts; strikes and eviction for `failed` and `unknown` including a legacy `probeFailures: undefined` and a budget of 1; purity.
...
- `adws/triggers/__tests__/pauseQueueScanner.test.ts` — all #903 cases unchanged (legacy path); new: probe not invoked when nothing is due; probe once with a mixed queue and the waiting entry untouched; `limited` with facts refreshes and never strikes; `limited` on a legacy entry gains the reset time; `limited` naming a limit type but no reset time writes only `lastProbeAt`; `clear` after reset resumes; eviction names `## Retry`, keeps the asserted phrase, writes no state; legacy JSON entry handled as before; injected clock stamps `lastProbeAt`.
...
- A `limited` probe that names a limit type but reports no reset time (feature-902's three-hour journey: a rejected `rate_limit_event` without `resetsAt`): a bare `refresh_reset`. The entry keeps its reset time and limit type (a legacy entry stays without both) and stays on the cadence path.
...
- `adws/triggers/pauseQueueDecider.ts` exists, is pure (no I/O, clock, env or logging imports), and its unit tests cover: before `resetsAt` → `skip_before_reset` for every verdict; `clear` after reset → `resume`; `limited` with a reported reset time → `refresh_reset` carrying it as ISO; `limited` without one → a bare `refresh_reset`, even when the probe names a limit type (no strike, no change to the reset time or the limit type); `failed`/`unknown` → `count_strike` until the budget, then `evict`.
...
- The `@adw-910` scenarios pass: all of `features/per-issue/feature-910.feature`, plus the `@adw-910` rows of `feature-902.feature` (including the renamed unknown-drop row's "## Retry" assertion) and of `feature-907.feature`.
...
- `NODE_OPTIONS="--import tsx" bunx cucumber-js --tags "@adw-910"` — `features/per-issue/feature-910.feature` plus the `@adw-910` rows of `feature-902.feature` and `feature-907.feature`, once the step-definition phase has defined the novel phrases (step 8).
...
- **Decision — `refresh_reset` is the single action for a confirmed limit.** The issue fixes the action set at five and lists "`limited` without a reset time → no strike, no change" as a case. Modelling that case as a bare `refresh_reset` keeps the contract at five actions. The shell writes only `lastProbeAt`, so the reset time, limit type and strike count are unchanged, including when the probe names a limit type without a reset time. No sixth "hold" action is introduced.
