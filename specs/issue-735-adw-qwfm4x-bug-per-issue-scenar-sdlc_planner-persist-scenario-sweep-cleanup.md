...
- `features/per-issue/feature-735.feature` — new BDD scenario file tagged `@adw-735` (plus an `@adw-{adwId}` tag per repo convention). Six scenarios: (§1) a stale per-issue scenario and its step-def sibling are BOTH removed from the worktree (Defect 2 / AC-a); (§2) the removal of both files is recorded in a commit on the default branch and the worktree is left with no uncommitted deletions (Defect 1 / AC-b); (§3) the removal is pushed so the origin remote's default branch no longer carries the scenario (AC-b); (§4) a scenario still inside the 14-day window is retained and the sweep produces no commit (AC-c, retention window unchanged); (§5) a second cron cycle over an already-persisted removal makes no further commit and leaves no uncommitted deletions (Defect 1 live symptom — no accumulation across cycles); (§6) the ADW TypeScript type-check passes (T22 backstop). All assertions target produced git artefacts (worktree file state, the deletion commit's name-status, working-tree cleanliness, the origin remote's default-branch tree, the type-checker's verdict), never source-file text — satisfying the rot-prevention rule.
...
- Create `features/per-issue/feature-735.feature` (header tags `@adw-735 @adw-qwfm4x-bug-per-issue-scenar` and a `Feature:` describing the persistent, sibling-aware sweep, plus a short observability/rot-prevention note — assert produced git artefacts only, no source-text reads, mirroring `feature-648`). Six scenarios, each driving the real sweep in-process over a fixture repo with a bare origin on the default branch (`feature-N.feature` + `feature-N.steps.ts` committed and pushed), and asserting only git artefacts:
  - §1 — orphaned sibling removed (Defect 2 / AC-a): given issue `#N` merged > 14 days ago, when the sweep runs, both `feature-N.feature` and `feature-N.steps.ts` are absent from the worktree.
  - §2 — removal persisted, tree left clean (Defect 1 / AC-b): the sweep commit on the default branch records the deletion of both files, and the worktree has no uncommitted deletions.
  - §3 — removal reaches the remote (AC-b): the origin remote's default branch no longer carries the swept scenario.
  - §4 — no-op guard / retention window unchanged (AC-c): a scenario merged `< 14 days` ago is retained (both files) and the sweep creates no new commit on the default branch.
  - §5 — no accumulation across cycles (Defect 1 live symptom): a second sweep over an already-persisted removal makes no further commit and leaves no uncommitted deletions.
  - §6 — type-check backstop (T22): the ADW TypeScript type-check passes after the change.
