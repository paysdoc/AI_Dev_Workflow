...
- `features/step_definitions/feature-701.steps.ts` — step definitions for any novel `@adw-701` phrases (reuse registered phrases and `feature-700`/`feature-691`/`feature-659` step defs verbatim where possible to avoid duplicate-definition errors).
...
- Create `features/per-issue/feature-701.feature` tagged `@adw-701 @adw-kojzv6-gitcontext-delete-pr`. Assert observable runtime behaviour only (mirror `feature-700.feature`'s rot-prevention preamble — no opening source files as text). Cover:
  - **§1 No process-global write on the auth path.** Driving an agent spawn through the new chokepoint with an injected launch-boundary context, the spawned child environment carries the context's `GH_TOKEN` and git identity, **and** a baseline snapshot of the parent `process.env` is byte-for-byte unchanged after the spawn (reuse #659/#700 recording-runner + parent-env-snapshot phrases verbatim where applicable; for the agent spawn, assert on a recorded child `env`).
  - **§2 The subprocess overlay overrides a foreign ambient token.** With a foreign `process.env.GH_TOKEN` seeded, an agent spawned with a context for `acme/webapp` runs with the context's token in its child env, never the foreign ambient value (the bleed made impossible at the subprocess boundary).
  - **§3 Guard passes whole-repo with zero allowlist.** `When the git/gh guard is run across the repository / Then the git/gh guard reports no violations` (reuse `feature-691` phrases) **and** `the git/gh guard reports zero allowlisted files` (reuse the `feature-700` §4b phrase / `(\d+) allowlisted == 0` observable).
  - **§4 No allowlist escape hatch.** A planted file containing a raw `git`/`gh` call **outside** `adws/gitContext/` makes the guard report a violation (exit 1), while an equivalent file **inside** `adws/gitContext/` is exempt — proving the package directory is the only exemption and there is no allowlist bypass. (Use a temp fixture path under the scanned tree, not an edit to a real source file; clean up in `After`.)
  - **§5 Type-check backstop (T22).** `the ADW TypeScript type-check passes` (reuse `feature-504` step).
- Create `features/step_definitions/feature-701.steps.ts` only for genuinely novel phrases; reuse registered/shared step defs (`feature-700`, `feature-691`, `feature-659`, `feature-504`) verbatim to avoid Cucumber duplicate-definition errors. Add a scoped `After` that restores any seeded `process.env.GH_TOKEN` and removes any planted fixture file.
