...
`features/per-issue/feature-793.feature` (`@adw-793`) is this slice's behavioural proof and the build's RED-first driver. Suggested coverage map:

- **AC1 (ready clone URL, ready identity)** → §1 an HTTPS clone URL handed to the workspace-ensure flow reaches `git clone` unchanged; §2 the adapter, not the core, produces the SSH form (including the dotted-name `paysdoc.nl` row); §3 the workspace-ensure flow performs no rewriting for a non-GitHub remote either.
- **AC2 (no `github.com` in core)** → §4 a source scan over `adws/gitContext/*.ts` (tests excluded — they legitimately hold remote-URL fixtures for `remoteUrl()`) finds no `github.com` literal, and the same scan over the adapter finds them. This is the load-bearing RED: `repoWorkspace.ts:63,66` and `bootstrapIdentity.ts:130,154,156` fail it today.
- **AC3 (zero ADW application imports)** → §5 a source scan over `adws/gitContext/*.ts` finds no import from `adws/core`, `adws/agents`, `adws/phases`, `adws/github`, `adws/triggers` or `adws/vcs`; §6 worktree create and remove messages arrive at an injected logger; §7 a context built with no logger completes without throwing. §5 must state its one documented exclusion in a comment: `gitContext.ts`'s five TRANSITIONAL `../providers/github/commands/*` builder imports, which #792 deliberately left and #796/#797 delete.
- **AC4 (behaviour unchanged)** → §8 identity resolution still answers App-bot → env → git-config → ADW Bot in that order, with the same strings; §9 `readLocalRepoInfo` still parses a dotted clone and still fails with `Failed to get repo info`; §10 the already-cloned branch still fetches and reads the default branch through the injected thunk. The "full suite green" half is carried by the unit suites and the merged proof tags, deliberately not by a scenario that shells out to the whole suite (the precedent set by #790 and #792 for the identical criterion).

Two consequences for the implementation: §6 requires the logger to be reachable through `GitContextDeps` alone, with no ambient state; and §4/§5 are source scans, so the split must be a real relocation — a re-export shim left inside `adws/gitContext/` would fail them.
...
- **Logger threading omission** — a missed call site silently falls back to `consoleLogger`, losing the timestamp/adwId prefix in production. The three-construction-site check in step 4 and §6's scenario are what catch it.
...
- [ ] Every `@adw-793` scenario is green, including the RED-before source-scan rows (§4 `github.com` in the core, §5 the application-logger imports).
