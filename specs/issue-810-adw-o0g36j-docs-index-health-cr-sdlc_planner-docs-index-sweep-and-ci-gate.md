...
- `features/per-issue/feature-745.feature` + `features/per-issue/step_definitions/feature-745.steps.ts` — the BDD pattern for driving an exported cron tick seam in-process (the `@adw-810` scenarios will follow it).
...
- `runDocsIndexSweep(deps)`: read index (null/empty → log + return empty report; a target repo without an index is a no-op); `assessDocsIndexHealth`; if repairs → `persistIndex(serializeConditionalDocs(repaired), repairs)` (log each repair); then reconcile: `findOpenDocsIndexReport(candidates)`; violations empty → close an open report if any; violations present and no report → `fileReport(buildDocsIndexReportIssue(...))`; report present with a different fingerprint → `refreshReport`; same fingerprint → `unchanged`. Each I/O step is wrapped so a transient git/gh failure is logged at `warn` and the sweep still returns its report — never throws.
...
- BDD (`@adw-810`, authored by the scenario writer, following `feature-745.feature`'s in-process seam pattern): the gate's exit code on a healthy vs. 20-dangling fixture directory; the sweep persisting exactly the 20 repairs through an injected `persistIndex`; at most one report issue across two consecutive sweeps with the same violations; a repaired-clean index closing the open report; the cron tick dispatching on-cadence only and surviving a throwing sweep; `bun run lint:git-guard` green after the allowlist entry removal.
...
- An open `chore/docs-index-sweep` PR from a failed prior merge → skipped this cycle (duplicate guard), retried next cadence because the repairs were never committed to the base.
