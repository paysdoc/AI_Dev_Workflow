...
- `features/per-issue/feature-776.feature` — **the acceptance contract, already authored** (`@adw-776 @adw-zbw0v7-webhook-server-dies`, **16 scenarios** in six sections). §1 containment — 5 scenarios (500 + alive + next delivery served + a 6-delivery retry storm + the `## Retry` twin), §2 reporting — 2 (log **and** Slack each naming event type, `paysdoc/paysdoc.nl`, issue 28), §3 the async half — 2 (alert is RED; survival is an already-GREEN guard; explicitly **no** 500), §4 non-distortion — 4 (400, 401, a no-work 200, and an ignored `issues`/`labeled` 200 keep their answers, no alert), §5 best-effort alerting — 2 (no endpoint / unreachable endpoint still 500 + survive), §6 type-check — 1. Every assertion targets a runtime artefact — the fix must not answer it with source-shape changes.
...
- **Coverage note:** this step has no `@adw-776` scenario, deliberately. `feature-776.feature` pins *per-event* containment (the issue's scope), and there is no hermetic way to make a health probe throw without adding a fault-injection hook to production code, which the feature's observability note forbids. It is verified by inspection plus the Step 7 wiring guard, and its only scenario-visible obligation is negative: the `.catch` must not change any existing answer.
...
- Add one wiring guard: the `req.on('end'` listener body contains `try {` and `containEventFailure(`, and the `/health` IIFE is terminated by a `.catch(`. Keep it to that — the two guarded wirings are exactly the ones no scenario can reach directly (the containment wiring is proven behaviourally by `@adw-776`, and the `/health` `.catch` of Step 5 has no scenario at all), and deeper source-shape assertions are exactly the rot this repo has already been bitten by.
...
- Hand the `generate_step_definitions` phase the harness facts verified in *Steps to Reproduce*: the mandatory `clone_url` in the payload, the present-but-empty credential env (dotenv does not overwrite), the failing `gh` stub on `PATH`, `SLACK_WEBHOOK_URL` (empty for the "no endpoint" case, sink URL otherwise), `GITHUB_WEBHOOK_SECRET` empty except for the 401 scenario, `cwd` at the repo root, a pre-seeded live-PID record at `agents/cron/paysdoc_paysdoc.nl.json` to keep `ensureCronProcess` from spawning a real cron (see below), the absent-`agents/.auth_gate` precondition, `setDefaultTimeout` above cucumber's 5 s default, and an `After` hook that always kills the spawned server.
- **The cron pre-seed is mandatory for these scenarios, not merely tidy.** The manual repro could pick a throwaway `repository.full_name`; the scenarios cannot — they carry the real incident repo `paysdoc/paysdoc.nl`, and §2/§3 assert on that exact string, so the name is load-bearing. Without the pre-seed every delivery carrying `repository` detach-spawns a real `trigger_cron.ts` against a real target repo — and in the four §4 scenarios that run *with working GitHub credentials* that cron would inherit them and start a real ADW loop. `AGENTS_STATE_DIR` is `process.cwd()/agents` and the server is spawned with `cwd` at the worktree root, so the seeded record is worktree-local and cannot disturb the live cron's records in the main checkout. `getCronPidFilePath` replaces only the `/` (`paysdoc/paysdoc.nl` → `paysdoc_paysdoc.nl.json`); the dot is preserved.
- Run `NODE_OPTIONS="--import tsx" bunx cucumber-js --tags "@adw-776"` and confirm all 16 scenarios pass.
...
- `NODE_OPTIONS="--import tsx" bunx cucumber-js --tags "@adw-776"` — all 16 scenarios in `features/per-issue/feature-776.feature` pass. §1/§2/§5 and the alert half of §3 are RED before the fix; §4, §6 and the survival half of §3 are GREEN before and must stay GREEN. Requires the step definitions authored by `generate_step_definitions`.
