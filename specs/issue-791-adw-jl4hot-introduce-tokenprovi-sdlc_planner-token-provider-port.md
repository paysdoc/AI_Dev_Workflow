...
- **The `@adw-791` counting assertions account for this probe, so do not "fix" a count by deleting it.** `features/per-issue/feature-791.feature` §2 expects the provider asked **4** times across `commitChanges`'s three commands (probe + three), §3 expects **2** asks for one command with that command carrying the provider's *second* answer, and §11/§12 expect the count to sit at **1** after construction alone. The probe is the only thing keeping the four construction-throw pins green; a zero-resolution construction would satisfy a stricter reading of "never cached" that the issue does not ask for ("cached", not "resolved") at the cost of deleting a fail-fast this slice was never asked to remove.
...
- **Coverage is fixed by the feature file's fourteen sections, not chosen here:** §1 two operations, two successive credentials; §2 `commitChanges`'s three commands, three resolutions, three distinct credentials; §3 construction probes once and discards, so the first command carries the provider's *second* answer; §4 a provider that stops answering fails the next command instead of replaying the last, and that command never reaches the spawn seam; §5 an ambient `GH_TOKEN`/`GITHUB_PAT` sentinel never reaches a child and never rescues a failed resolution; §6 an elevated operation carries whatever the provider answers, and the core still declares the class; §7 elevated operations get the PAT and ordinary ones the installation mint, driven through the real `createGitHubTokenProvider`; §8 with no PAT configured an elevated operation falls back to the resolved token; §9 the resolution order App → PAT → `gh auth token` → loud throw, asserted through a spawned command; §10 two contexts, two repositories, credentials bound to their own; §11 a credential handed to the public `exec` is not replaced and the executor does not consult the provider; §12 the launch boundary hands over the provider, not a resolved string; §13 no process-environment mutation; §14 the type-check backstop (T22, reused).
- **Two shape mappings the step definitions must make.** (a) `the GitHub credential source is configured with … the personal access token {string}` names **one** PAT, but the provider gives it two roles — set **both** `pat` and `alternateIdentityPat` from that value, mirroring `gitContextForRepo`, or §7's elevated rows go green vacuously while §9's order rows stop seeing the PAT as the second candidate. (b) Build contexts **lazily**: every scenario configures its credential source in a Given that *follows* the context Given, so `new GitContext(...)` must be deferred to first use. A source that can produce no credential then surfaces its failure at the first operation (§5's second row, §9's two failure rows) rather than throwing inside a Given.
