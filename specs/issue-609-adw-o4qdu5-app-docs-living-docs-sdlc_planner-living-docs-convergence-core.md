...
Prove the behavior with a per-issue BDD scenario file (`features/per-issue/feature-609.feature`, tagged `@adw-609`): content-assertions that the `/document` prompt now instructs rewrite-in-place / glob-match / update-single-entry / never-append (and dropped the append-only instruction), plus behavioral assertions driving the registry module in-process (parse/serialize round-trip; two upserts of one owned area → exactly one doc + one entry; novel area → a new entry).
...
- `features/per-issue/feature-609.feature` — Per-issue BDD scenario (`@adw-609`): content-assertions on the rewritten `/document` prompt + behavioral assertions on the registry module (convergence: two runs → one doc + one entry).
...
- Create `features/per-issue/feature-609.feature` (tagged `@adw-609`, following the `feature-506` shape), with:
  - **Content-assertion scenario(s)** on `.claude/commands/document.md`: the prompt instructs rewriting the owning module doc **in place**, matching by **explicit file globs**, updating the **single** existing entry, and **never appending** a second entry for an already-owned area; it describes a **current-state** doc (no changelog/history); and it **no longer** contains the append-only "Add an entry for the new documentation file" instruction.
  - **Behavioral scenario(s)** on the registry module (in-process, artefact-style — not asserting source structure): given an index serialized from one entry owning glob `adws/foo/**` with doc `app_docs/foo.md`, when the convergence primitive resolves ownership for a changed file `adws/foo/bar.ts` and upserts that area **twice**, then the registry contains **exactly one** entry for `app_docs/foo.md` (one doc path, no duplicate); and given a changed file under no owned glob, an upsert for a new doc path appends **exactly one** new entry. Include a parse→serialize round-trip assertion.
- Create `features/per-issue/step_definitions/feature-609.steps.ts`:
  ...
  - For content-assertions, read `.claude/commands/document.md` from the framework root and assert presence/absence of the convergence semantics.
  - For behavioral steps, import the registry module from `adws/core/conditionalDocsRegistry.ts` and exercise `parseConditionalDocs` / `serializeConditionalDocs` / `findOwningEntry` / `upsertEntry` directly, asserting the convergence outcomes.
