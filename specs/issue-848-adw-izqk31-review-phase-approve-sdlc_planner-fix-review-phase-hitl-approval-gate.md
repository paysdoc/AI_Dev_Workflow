...
- The scenario-writer phase has already produced `features/per-issue/feature-848.feature` (tags `@adw-848 @adw-izqk31-review-phase-approve`, fixture repo `adw-fixture/void-848`, issue 42 / pull request 7, a `Background` of `the ADW codebase is checked out` (G18 no-op, registered in `features/step_definitions/ensureCronOnEveryEventSteps.ts`) plus the 796 recording-boundary phrase). Treat it as the contract; it is frozen by `gherkinFreeze` during the fix loop. Its six rows and what each proves:
  ...
  6. **§4 "The hitl merge gate still defers the unapproved pull request after the review phase passes"** (AC5, regression proof) — `the recording code host counts the approvals it submits as approving reviews`; `issue 42 carries the label "hitl"`; branch `feature-issue-42-void` has open PR 7 with no approving review; merge deps built from the same boundary; review passes; `the merge orchestrator runs for issue 42 under adw id "izqk31-void-merge"` → `reports the outcome "abandoned" for reason "hitl_blocked_unapproved"` and `recorded no approval`. Before the fix the review pass approves PR 7, the flag turns that into an approving review, and `executeMerge` walks past the gate (any outcome other than `hitl_blocked_unapproved`), so the row is RED for the exact production reason. This row is the issue's "regression scenario [that] proves the `hitl` gate survives a review pass" (AC5), and it discharges AC5 as a per-issue row under `@adw-848`. The feature file names it "the candidate for promotion into the regression suite; promotion is a human decision", and `.adw/scenarios.md` configures a Regression Scenario Directory and Vocabulary Registry, so nothing is auto-promoted. Leave promotion to a human: do not tag the row `@regression`, move it into `features/regression/`, or register its phrases in `features/regression/vocabulary.md` as part of this fix.
- Phrase resolution, per row (verified by a dry run during alignment). The two `Background` phrases already resolve: `the ADW codebase is checked out` → `features/step_definitions/ensureCronOnEveryEventSteps.ts:8`, and the recording-boundary phrase → `feature-796.steps.ts:610`. Every other phrase outside the seven in Step 5 already resolves to `feature-796.steps.ts` / `feature-820.steps.ts`. The Step 5 phrases each row needs are:
  - row 1: the issue-record label Given, the logger-capture Given and the captured-log Then
  - row 2: the issue-record no-labels Given
  - row 3: none
  - row 4: the failed-approval flag Given
  - row 5: the refused-probe flag Given
  - row 6: the approvals-count-as-reviews flag Given

  Today `NODE_OPTIONS="--import tsx" bunx cucumber-js --tags "@adw-848" --dry-run` reports exactly those seven steps undefined and no ambiguous step (6 scenarios: 5 undefined, 1 skipped). After Step 5 the same dry run must report 0 undefined and 0 ambiguous; an ambiguous result means a phrase was re-registered.
