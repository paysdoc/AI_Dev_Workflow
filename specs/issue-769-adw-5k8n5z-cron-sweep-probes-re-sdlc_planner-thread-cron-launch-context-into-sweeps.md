...
1. **Thread the launch context.** `PerIssueSweepDeps` and `PromotionSweepDeps` each gain a **required** `gitContext: GitContext` field, and `deps` stops being optional. Every default becomes a closure over that one context (`makeDefaultDeps(ctx)` for the promotion sweep; `prepareSweepBase(ctx)` + a curried `defaultGetMergedAt(ctx, n)` for the per-issue sweep). `getRepoInfo` and `gitContextForRepo` are **deleted from all three sweep files**, so **zero** contexts are constructed per pass — the single launch context is reused. Where a `RepoInfo` is still needed by an un-migrated collaborator (`mergePR`, `defaultFindPRByBranch`, `applyLabel`), derive it from the context's own identity accessors (`gitContext.ts:124-125`), never from cwd.
...
### New Files
- `features/per-issue/feature-769.feature` — the `@adw-769` BDD regression scenario (authored by the scenario-writer agent from §10 below).
- `features/per-issue/step_definitions/feature-769.steps.ts` — its self-contained step definitions.
...
- **Introduce no allowlist** — #701 deleted it deliberately. Step 6 leaves zero legitimate violations.
...
### 10. Add the `@adw-769` BDD regression scenario
`features/per-issue/feature-769.feature` + `features/per-issue/step_definitions/feature-769.steps.ts`, self-contained (own module-private `ctx` and `After({ tags: '@adw-769' })` hook), reusing only the registered `the ADW codebase is checked out` (G18) and `the ADW TypeScript type-check passes` (T22). Assert on injected-seam recordings and real fixture-repo artefacts — **never** by reading `trigger_cron.ts`/`promotionSweep.ts` as text. Harness model: `feature-740.steps.ts:64-80` (real temp git repo + real bare `origin`, `GitContext` built directly from `GitContextOptions`, no network, no real `gh`).

Sections to cover:
1. **The per-issue sweep runs against the launch context, not cwd** — drive `runPerIssueScenarioSweepTick` on a cadence-eligible cycle with a capturing thunk bound to a target-shaped context; assert the sweep received exactly that context (owner/repo/basePath = the target's), not the framework's.
2. **Same for the promotion sweep** via `runPromotionSweepTick`.
3. **No launch context → skipped, not retargeted** (Scenario Outline over both sweeps): with a `null` thunk on a cadence-eligible cycle, the sweep is not dispatched and the tick completes without raising.
4. **The cross-repo proof (the headline).** Build a **target-shaped** fixture `GitContext` (`selfHost: false`, `targetReposDir` = a temp dir, `owner`/`repo` chosen so `join(targetReposDir, owner, repo)` is the seeded fixture repo) whose `frameworkRepoRoot` points at a **separate decoy framework checkout** seeded with its own `features/per-issue/feature-{M}.feature`. Drive the real `runPromotionSweep({ gitContext: fixtureCtx, … })` with only the network seams injected (`listPromotionIssues`, `loadStats`, `fileIssue`, and a capturing `tagAndCommit`), letting `listPerIssueFeatures` / `readFeatureContent` / `scenariosConfig` / `loadVocabulary` fall through to the real defaults. Assert the returned `PromotionSweepReport` references **only** the fixture's feature numbers, that no decoy path appears anywhere in it, and that the decoy file is byte-unchanged. Pre-fix this is RED for the strongest possible reason: the defaults would list *this repo's* 94 per-issue features instead of the fixture's.
5. **Self-host is not a regression** — the same drive with a self-host-shaped context (`selfHost: true`, `frameworkRepoRoot` = the fixture) still sweeps the fixture.
6. **Type-check backstop (T22)** — the ADW type-check passes with the required `gitContext` threaded through both sweeps, both tick seams, and the updated #740/#741 step defs.
...
- `NODE_OPTIONS="--import tsx" bunx cucumber-js --tags "@adw-769"` → **RED before the fix** (§4's real-defaults drive lists this repo's `features/per-issue/` instead of the fixture's; §1-§3 fail because the sweeps take no context). **GREEN after.**
...
- **Why the tick seam keeps `sweep` in position 2.** Putting the context in position 2 would break `feature-745.steps.ts:78` and force a rewrite of a passing regression scenario. A nullable bound thunk satisfies every acceptance criterion (skip-not-fallback is drivable by passing `null`) at zero churn.
