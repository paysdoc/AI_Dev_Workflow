...
- **Comment-only guard** — `checkCommentOnly.ts` (`bun run lint:comment-only [--base <ref>] <files...>`) proves that a given set of `.ts`/`.tsx`/`.feature` files differs from a base ref (default: `origin/<defaultBranch>`) only in comments, JSDoc, and whitespace — tokenizes TypeScript via the `typescript` compiler API and folds Gherkin doc-strings, so a genuine code change fails the check even if it's dressed up as a comment tweak.
...
├── checkCommentOnly.ts  # Comment-only guard: proves a file set differs from a base ref only in comments/whitespace (`bun run lint:comment-only`)
