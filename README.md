...
- **Comment-only diff guard** — `checkCommentOnly.ts` (`bun run lint:comment-only [--base <ref>] <files...>`) proves a set of changed files differs from a base ref only in comments, JSDoc, and whitespace (TypeScript via the compiler API's leaf tokens, `.feature` files via a Gherkin-DocString-aware line fold); it backs the comment de-bloat PRD's per-file sweep batches — not wired into CI, run per sweep — and fails on any unsupported, missing, or substantively-changed file.
...
├── checkCommentOnly.ts  # Per-sweep guard: proves a diff differs from a base ref only in comments/JSDoc/whitespace (`bun run lint:comment-only`)
