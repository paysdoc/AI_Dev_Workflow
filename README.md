...
- **Comment-only diff guard** — `adws/checkCommentOnly.ts` (`bun run lint:comment-only [--base <ref>] <files...>`) proves a set of files differs from a base ref only in comments/JSDoc and whitespace, by parsing TS/TSX with the TypeScript compiler API's leaf tokens or folding `.feature` files line-by-line (Gherkin-DocString-aware); backs comment-debloat sweep batches per-file rather than running as a permanent CI gate.
...
├── checkCommentOnly.ts  # Comment-only diff guard: proves a file set differs from a base ref only in comments/whitespace (`bun run lint:comment-only`), backing comment-debloat sweep batches
