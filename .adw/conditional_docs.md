...
- app_docs/feature-oqb76h-gitcontext-base-path-authority.md
  ...
  - Conditions:
    ...
    - When the per-command env injection classifiers (`#run`/`#runRepoApi`, both delegating to `exec()`), `usePat`, or stdin `input` forwarding is relevant — `usePat` is GitHub-specific, private to the classifiers, and does not appear in `exec()`'s public signature (leaves in the TokenProvider slice, #791)
    ...
    - When adding unit tests for `adws/gitContext/` (env-injection, non-mutation, `usePat`, lease-rejection, two-context isolation, token veracity, bootstrap identity, workspace management)
