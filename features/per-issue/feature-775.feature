...
Feature: Repo-API gh commands run from a working directory that always exists — a Cancel or Retry directive is processable on a host that has never cloned the target workspace
  ...
  Scope notes:
    ...
    • `gh pr create` IS THE ONE COMMAND WITH KNOWN CWD SENSITIVITY and is deliberately NOT pinned in
      §6. Outside a git repository `gh pr create` requires an explicit `--head`; ADW always passes one
      (`prCommands.ts:44-51`), so it is safe to move, but the claim rests on gh CLI behaviour rather
      than on anything this repo controls, so it stays a maintainer decision rather than a hard
      assertion. Same reasoning applies to nothing else: every other write is a plain API call.
    • THIS FIX SUPERSEDES A CONTRACT SEVEN EXISTING PER-ISSUE SCENARIOS ASSERT. `the captured command
      ran with cwd equal to the context base path` is asserted about GH operations at
      feature-659.feature:225, :240, :305, :307 (`default-branch`); feature-663.feature:221, :237
      (outline), :337, :339; feature-691.feature:246, :261 (outline, gh-reads);
      feature-695.feature:337 (`gh secret set`); feature-697.feature:37 (`pr-changed-files`); and
      feature-700.feature:342 (`default-branch`). Those scenarios encode the PRE-#775 contract and
      will go RED when this fix lands. They are NOT tagged `@adw-775`: tagging them would pull
      assertions of the superseded contract into this issue's proof run, which can then never go
      green. Rewriting another issue's scenarios also requires rewriting their step definitions, which
      is not this agent's output. THE IMPLEMENTER MUST MIGRATE THEM AS PART OF THIS FIX — the natural
      migration is to keep the token/git-identity assertions and replace the gh-op cwd assertion with
      the framework-root form. The same superseded contract lives in unit tests:
      `adws/gitContext/__tests__/gitContextOperations.test.ts:52` (`defaultBranch`), `:195`
      (`listOpenIssues`), `:252` (`issueComments`), `:317` (`fetchMergedPRs`), `:852` (`setSecret`),
      `:899` (`runGraphQLInput`). Unaffected, because their ops are git or self-host:
      feature-661, feature-662, feature-694:351, feature-696, feature-698, feature-699:360
      (self-host, where basePath IS the framework root).
  ...
  Vocabulary note:
    ...
    directories, because the failure being fixed is a real spawn against a real missing path):
      ...
      • §5's chdir step must `process.chdir` to a third temp dir and restore the original cwd in
        `After` (feature-769's precedent).
  ...
  # ── §4 THE DIRECTORY EXISTS AND IS NOT THE WORKSPACE (DB 1) ─────────────────────────────
  #
  # The two properties the issue actually requires, asserted directly on the recorded spawn
  # parameters rather than through any particular choice of directory. RED before on both counts:
  # pre-clone, basePath neither exists nor is anything other than the workspace.

  @adw-775 @adw-uvv9hm-repo-api-gh-commands
  Scenario: A repo-API command runs from a directory that exists and is not the target workspace
    Given a target repository "acme/webapp" registered on a host that has never cloned its workspace
    And the context's spawned commands are recorded by a spawn recorder
    When the issue comments for issue 28 are fetched through the context
  ...
  # ── §8 GIT COMMANDS KEEP BASEPATH (DB 2) ────────────────────────────────────────────────
  #
  # The unchanged half of the contract. A git read is genuinely workspace-scoped, so it still runs
  # from the target workspace directory. GREEN before and after — the guard against a wholesale
  # change to `#run`'s default cwd.

  @adw-775 @adw-uvv9hm-repo-api-gh-commands
  Scenario: A workspace-scoped git command still runs from the target repository's workspace directory
    Given a target repository "acme/webapp" registered on a host that has already cloned its workspace
    And the context's spawned commands are recorded by a spawn recorder
